...
int stui_check_mode(int checking_mode)
{
	int current_mode = stui_get_mode();

	pr_debug(TUIHW_LOG_TAG " %s %#X\n", __func__, current_mode);
	return (current_mode & checking_mode);
}
EXPORT_SYMBOL(stui_check_mode);
...
int stui_cancel_session(void)
{
	...
	if (atomic_cmpxchg(&canceling, 0, 1) != 0) {
		...
		while (stui_check_mode(STUI_MODE_ALL) && (count < MAX_WAIT_CNT)) {
