...
static struct task_struct *iwd_kthread;
static struct completion finished;
static int reboot_notif_registered;
static int tzdev_notif_registered;
...
static int get_display_info(GetDisplayInfo_cmd_t *cmd, GetDisplayInfo_rsp_t *rsp)
{
	...
	for (i = 0; i < STUI_DISPLAY_INFO_SIZE; i++)
		pr_debug(TUIHW_LOG_TAG " rsp->lcd_info[%d]  =%llx\n", i, rsp->lcd_info[i]);
...
static void reboot_phone(void)
{
	...
	pr_err(TUIHW_LOG_TAG " Trusted User Interface was not unlocked.");
	BUG_ON(1);
...
static int connecting_thread(void *data)
{
	...
	while (atomic_cmpxchg(&thread_flag, 1, 1) == 1
		...
		cmd.task_id    = -1;
		cmd.SetDrvInfo_cmd.drv_tui_mode = stui_check_mode(STUI_MODE_ALL);
