...
static inline int tzdev_scma_free_contig(dma_addr_t base, unsigned int size)
{
	(void)base;
	(void)size;
...
static int tz_scma_init_call(struct notifier_block *cb, unsigned long code, void *unused)
{
	...
	ret = tzdev_smc_channel_read(ch, &mem_chunk, sizeof(struct scma_iwio_mem_chunk_desc));
	if (ret) {
		log_error(tzdev_scma, "Failed to get to read memory chunk, error=%d\n", ret);
		tzdev_smc_channel_release(ch);
		goto free;
	}
	...
	ret = tzdev_smc_channel_write(ch, &mem_chunk, sizeof(struct scma_iwio_mem_chunk_desc));
	if (ret) {
		log_error(tzdev_scma, "Failed to write to smc channel, error=%d\n", ret);
		tzdev_smc_channel_release(ch);
		goto free;
	}
