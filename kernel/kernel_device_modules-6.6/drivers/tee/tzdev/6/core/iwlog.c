...
static DEFINE_PER_CPU(struct tzdev_iw_mem *, tz_iwlog_buffer);
...
static void tz_iwlog_read_buffers(void)
{
	...
	for_each_possible_cpu(i) {
		struct tzdev_iw_mem *b = per_cpu(tz_iwlog_buffer, i);
		if (!b)
			continue;

		buf = tzdev_iw_mem_get_map_address(b);
...
static int tz_iwlog_panic_handler(struct notifier_block *self, unsigned long v, void *p)
{
	(void)self;
	(void)v;
	(void)p;

	log_debug(tzdev_iwlog, "Kernel panic event received.\n");
	tz_iwlog_read_buffers();

	return 0;
}

static struct notifier_block tz_iwlog_panic_notifier = {
	.notifier_call = tz_iwlog_panic_handler,
};
...
int tz_iwlog_init(void)
{
	...
	for (i = 0; i < NR_SW_CPU_IDS; ++i) {
		struct tzdev_iw_mem *buffer;

		buffer = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_LOG,
						    CONFIG_TZLOG_PG_CNT * PAGE_SIZE, NULL, NULL);
		if (IS_ERR(buffer)) {
			log_error(tzdev_iwlog, "Failed to connect iwlog (error %ld)\n", PTR_ERR(buffer));
			return PTR_ERR(buffer);
		}

		// Save iw_mem itself to make kmemleak happy
		if (cpu_possible(i))
...
#endif
	ret = atomic_notifier_chain_register(&panic_notifier_list, &tz_iwlog_panic_notifier);
	if (ret) {
		log_error(tzdev_iwlog, "Failed to register panic notifier, error=%d\n", ret);
...
void tz_iwlog_fini(void)
{
	...
	tzdev_atomic_notifier_unregister(TZDEV_PRE_SMC_NOTIFIER, &tz_iwlog_pre_smc_notifier);
#endif
	atomic_notifier_chain_unregister(&panic_notifier_list, &tz_iwlog_panic_notifier);
