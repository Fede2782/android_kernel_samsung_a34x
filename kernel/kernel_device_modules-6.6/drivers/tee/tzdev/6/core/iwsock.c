...
#define DEFAULT_TRANSMIT_BUFFER_SIZE						\
	round_down(								\
		(PAGE_SIZE - 							\
			round_up(sizeof(struct iwd_sock_buf_head), CIRC_BUF_ALIGN) -	\
			3 * round_up(CIRC_BUF_META_SIZE, CIRC_BUF_ALIGN) -	\
			3 * circ_buf_size_for_packet(sizeof(struct tz_cmsghdr_cred)) -	\
			3 * circ_buf_size_for_packet(0) -			\
			DEFAULT_OOB_BUFFER_SIZE) / 2,				\
		CIRC_BUF_ALIGN)
...
#define GET_SOCKET_RECEIVE_BUF(iwd_buf, snd_buf_size)				\
	(struct circ_buf *)((char *)iwd_buf + 					\
		round_up(sizeof(struct iwd_sock_buf_head), CIRC_BUF_ALIGN) + 	\
		tz_iwsock_buf_total_size(snd_buf_size))

#define GET_SOCKET_OOB_BUF(iwd_buf, rcv_buf_size, snd_buf_size)			\
	(struct circ_buf *)((char *)iwd_buf + 					\
		round_up(sizeof(struct iwd_sock_buf_head), CIRC_BUF_ALIGN) +	\
		tz_iwsock_buf_total_size(snd_buf_size) +			\
		tz_iwsock_buf_total_size(rcv_buf_size))
...
struct iwd_sock_events_buf {
	...
	struct iwd_sock_events_stream_buf swd_close_events;
...
static struct tzdev_iw_mem *sock_events;
...
static inline struct iwd_sock_events_buf *iw_event_get_events_buf(void)
{
	return tzdev_iw_mem_get_map_address(sock_events);
}
...
{
	tzdev_iw_mem_destroy(sd->iwd_buf);
...
static int tzdev_iwsock_check_limit_cb(unsigned int cntr)
{
	return (cntr >= MAX_SOCK_PER_CLIENT) ? -ENOSPC : 0;
}
...
static int tz_iwsock_publish_sd(struct sock_desc *sd)
{
	...
	ret = tzdev_inc_id_cntr(LIMIT_TYPE_IW_SOCK, sd->client_task, tzdev_iwsock_check_limit_cb);
	if (ret) {
		mutex_unlock(&tzdev_sock_map_lock);
		return ret;
	}
	...
	if (sid >= 0) {
		...
		tzdev_dec_id_cntr(LIMIT_TYPE_IW_SOCK, sd->client_task);
...
static unsigned long circ_buf_total_size(unsigned long size)
{
	return round_up(size + CIRC_BUF_META_SIZE, CIRC_BUF_ALIGN);
}

static unsigned long tz_iwsock_buf_total_size(unsigned long size)
{
	return circ_buf_total_size(tz_iwsock_buffer_size(size));
}

static unsigned long connected_iwd_buf_total_size(struct sock_desc *sd)
{
	return round_up(round_up(sizeof(struct iwd_sock_buf_head), CIRC_BUF_ALIGN) +
			tz_iwsock_buf_total_size(sd->snd_buf_size) +
			tz_iwsock_buf_total_size(sd->rcv_buf_size) +
			tz_iwsock_buf_total_size(sd->oob_buf_size),
			PAGE_SIZE);
}
...
static int tz_iwsock_pre_connect_callback(struct tzdev_iw_mem *buf,
		unsigned long num_pages, void *ext_data)
{
	struct connect_ext_data *data = ext_data;
	struct sock_desc *sd = data->sd;
	struct iwd_sock_buf_head *iwd_buf = tzdev_iw_mem_get_map_address(buf);
	...
	/* Write buffer */
	circ_buf_connect(&sd->write_buf, circ_buf_set(snd_buf), tz_iwsock_buffer_size(sd->snd_buf_size));

	/* Receive buffer */
	circ_buf_connect(&sd->read_buf, circ_buf_set(rcv_buf), tz_iwsock_buffer_size(sd->rcv_buf_size));
	...
	case ACCEPT_FROM_NWD:
		...
		circ_buf_connect(&sd->oob_buf,
				circ_buf_set(oob_buf), tz_iwsock_buffer_size(sd->oob_buf_size));
...
{
	if (tz_event_add((struct iwd_events_buf *)&iw_event_get_events_buf()->nwd_events, sid))
...
struct sock_desc *tz_iwsock_socket(unsigned int is_kern, unsigned int is_interruptible)
{
	...
	sd->client_task = tzdev_get_client_task();
...
int tz_iwsock_connect(struct sock_desc *sd, const char *name, int flags)
{
	...
	buf = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_SOCKET,
			connected_iwd_buf_total_size(sd), tz_iwsock_pre_connect_callback, &ext_data);
	...
	if (flags & MSG_DONTWAIT) {
		smp_rmb();
		/* Non-blocking mode */
		if (iwsock_buf_head(sd)->swd_state != BUF_SK_CONNECTED)
			ret = -EINPROGRESS;
		else
...
static int tz_iwsock_is_connection_done(struct sock_desc *sd)
{
	...
	return sd->state != TZ_SK_CONNECT_IN_PROGRESS ||
			iwsock_buf_head(sd)->swd_state != BUF_SK_NEW;
}
...
int tz_iwsock_wait_connection(struct sock_desc *sd)
{
	...
	switch (iwsock_buf_head(sd)->swd_state) {
	case BUF_SK_CONNECTED:
	case BUF_SK_CLOSED:
		/* Mark state as connected even if SWd already closed their endpoint.
		 * This will allow to read data that could be written by SWd before
		 * close of new socket. */
		sd->state = TZ_SK_CONNECTED;
		ret = 0;
		break;
...
int tz_iwsock_listen(struct sock_desc *sd, const char *name)
{
	...
	buf = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_SOCKET,
			listen_iwd_buf_total_size(sd),
			tz_iwsock_pre_connect_callback, &ext_data);
...
EXPORT_SYMBOL(tz_iwsock_listen);
...
struct sock_desc *tz_iwsock_accept(struct sock_desc *sd)
{
	...
	iwd_buf = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_SOCKET,
			connected_iwd_buf_total_size(new_sd),
			tz_iwsock_pre_connect_callback, &ext_data);
	if (IS_ERR(iwd_buf)) {
		new_sd->iwd_buf = NULL;
		ret = PTR_ERR(iwd_buf);
...
socket_allocation_failed:
	...
	if (tz_event_add((struct iwd_events_buf *)&iw_event_get_events_buf()->accept_err_events,
			conn_req.swd_id))
		tz_kthread_pool_enter_swd();

	return ERR_PTR(ret);
}
EXPORT_SYMBOL(tz_iwsock_accept);

void tz_iwsock_release(struct sock_desc *sd)
{
	unsigned int notify_swd = 0;
	struct iwd_sock_connect_request conn_req;
	...
	case TZ_SK_LISTENING:
		...
		iwsock_buf_head(sd)->nwd_state = BUF_SK_CLOSED;

		smp_mb();

		/* Read all requests and notify SWd IDs back to wake waiting threads */
		while (iwsock_buf_head(sd)->swd_state != BUF_SK_CLOSED &&
				circ_buf_read(&sd->read_buf, (char *)&conn_req,
				sizeof(conn_req), CIRC_BUF_MODE_KERNEL) > 0)
			tz_event_add((struct iwd_events_buf *)&iw_event_get_events_buf()->accept_err_events,
					conn_req.swd_id);
	...
	case TZ_SK_CONNECTED:
		...
		if (iwsock_buf_head(sd)->swd_state != BUF_SK_CLOSED)
			notify_swd = 1;
			...
			unsigned int *need_notify)
{
	...
	swd_state = iwsock_buf_head(sd)->swd_state;
...
retry:
	...
	if (sd->max_msg_size) {
		nbytes = tz_iwsock_buffer_size(sd->max_msg_size) * 2;
...
static void tz_iwsock_handle_close_event(unsigned int id)
{
	struct sock_desc *sd;
...
static int tz_iwsock_kthread(void *data)
{
	...
	while (!kthread_should_stop()) {
		wait_event_uninterruptible_freezable_nested(tz_iwsock_wq,
				tz_event_get_num_pending((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_events) > 0 ||
				tz_event_get_num_pending((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_close_events) > 0 ||
				kthread_should_stop());

		if (kthread_should_stop())
			return 0;

		tz_event_process((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_close_events,
				tz_iwsock_handle_close_event);

		tz_event_process((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_events,
...
static int tz_iwsock_events_pre_connect_callback(struct tzdev_iw_mem *buf, unsigned long num_pages, void *data)
{
	struct iwd_sock_events_buf *sock_buf = tzdev_iw_mem_get_map_address(buf);
	...
	tz_event_init((struct iwd_events_buf *)&sock_buf->swd_close_events, MAX_NUM_SOCKETS);
...
static int tz_iwsock_post_smc_call(struct notifier_block *cb, unsigned long code, void *unused)
{
	...
	if (tz_event_get_num_pending((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_events) > 0 ||
			tz_event_get_num_pending((struct iwd_events_buf *)&iw_event_get_events_buf()->swd_close_events) > 0)
...
int tz_iwsock_init(void)
{
	...
	sock_events = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_SOCKET_EVENTS,
						IWD_SOCKET_EVENTS_BUF_NUM_PAGES * PAGE_SIZE,
						tz_iwsock_events_pre_connect_callback,
						NULL);
	if (IS_ERR(sock_events)) {
		ret = PTR_ERR(sock_events);
		log_error(tzdev_iwsock, "Failed to pack iw memory, error=%d\n", ret);
		return ret;
	}
...
kthread_start_failed:
	tzdev_iw_mem_destroy(sock_events);
