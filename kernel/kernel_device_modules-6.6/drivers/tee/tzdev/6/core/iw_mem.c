...
/**
 * tzdev_iwio_channel_connect() - Connect shared memory via smc channel
 * @mode: connect mode
 * @size: shrared memory size (will be rounded up to PAGE_SIZE)
 * @cb: pre connection callback. Will be called right before calling smc
 * @data: callback optional argument
 *
 * Return: iw_mem descriptor or ERR_PTR on failure
 */
struct tzdev_iw_mem *tzdev_iwio_channel_connect(int mode, size_t size, tzdev_smc_channel_preconnect_cb cb, void *data)
{
	...
	if (IS_ERR(mem)) {
		log_error(tzdev_iw_mem, "Failed to create iw memory (error %ld)\n", PTR_ERR(mem));
		return mem;
	}
	...
	if (!buffer) {
		...
		log_error(tzdev_iw_mem, "Failed map iw memory\n");
		return ERR_PTR(-EPERM);
	}

	if (cb) {
		ret = cb(mem, PAGE_ALIGN(size) / PAGE_SIZE, data);
		if (ret) {
			tzdev_iw_mem_destroy(mem);
			return ERR_PTR(ret);
		}
	...
	if (ret) {
		...
		tzdev_iw_mem_destroy(mem);
		log_error(tzdev_iw_mem, "Failed to pack iw memory (error %d)\n", ret);
		return ERR_PTR(ret);
	}

	ret = tzdev_smc_connect(mode);
	tzdev_smc_channel_release(ch);
