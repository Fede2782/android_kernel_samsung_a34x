...
#define MAX_SHMEM_PER_CLIENT	64
...
static int tzdev_iw_shmem_check_limit_cb(unsigned int cntr)
{
	return (cntr >= MAX_SHMEM_PER_CLIENT) ? -ENOSPC : 0;
}
...
static int tzdev_iw_shmem_map_add(struct tzdev_iw_shmem *shmem)
{
	...
	ret = tzdev_inc_id_cntr(LIMIT_TYPE_IW_SMEM, shmem->client_task, tzdev_iw_shmem_check_limit_cb);
	if (ret) {
	...
	if (ret < 0)
		tzdev_dec_id_cntr(LIMIT_TYPE_IW_SMEM, shmem->client_task);
	mutex_unlock(&tzdev_iw_shmem_map_lock);
		...
		unsigned int flags)
{
	...
	shmem->state = CREATED;
	shmem->mem = mem;
	shmem->client_task = tzdev_get_client_task();
