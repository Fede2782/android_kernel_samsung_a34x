...
static int tzdev_sysconf_init(void)
{
	...
	ret = tzdev_smc_channel_write(ch, &tz_sysconf.nwd_sysconf, sizeof(struct tzio_nwd_sysconf));
	if (ret) {
		log_error(tzdev, "tzdev_smc_channel_write() failed with %d\n", ret);
		goto out;
	}
	...
	ret = tzdev_smc_channel_read(ch, &tz_sysconf.swd_sysconf, sizeof(struct tzio_swd_sysconf));
	if (ret)
		log_error(tzdev, "tzdev_smc_channel_read() failed with %d\n", ret);
