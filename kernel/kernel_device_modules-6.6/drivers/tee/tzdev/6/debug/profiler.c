...
struct profiler_buf_entry {
	...
	struct tzdev_iw_mem *tzio_buf;
};

static inline struct circ_buf *profiler_get_tzio_circ(const struct profiler_buf_entry *e)
{
	return tzdev_iw_mem_get_map_address(e->tzio_buf);
}
...
static int tzprofiler_init_buf_pool(unsigned int bufs_cnt, unsigned int buf_pg_cnt)
{
	...
	for (i = 0; i < bufs_cnt; ++i) {
		...
		if (profiler_buf == NULL)
			return -ENOMEM;
		profiler_buf->tzio_buf = tzdev_iwio_channel_connect(TZ_IWIO_CONNECT_PROFILER,
								   buf_pg_cnt * PAGE_SIZE,
								   NULL, NULL);
		if (IS_ERR(profiler_buf->tzio_buf)) {
			int ret = PTR_ERR(profiler_buf->tzio_buf);

			kfree(profiler_buf);
			return ret;
		}
		profiler_get_tzio_circ(profiler_buf)->write_count = profiler_get_tzio_circ(profiler_buf)->read_count = 0;
...
static int tzprofiler_set_uuid(struct tz_uuid *uuid)
{
	...
	ret = tzdev_smc_channel_write(ch, uuid, sizeof(struct tz_uuid));
	if (ret) {
...
static int tzprofiler_set_start_addr(uint64_t *addr)
{
	...
	ret = tzdev_smc_channel_write(ch, addr, sizeof(uint64_t));
	if (ret) {
		tzdev_smc_channel_release(ch);
		return ret;
	}
...
static int tzprofiler_set_steps_number(uint32_t steps)
{
	...
	ret = tzdev_smc_channel_write(ch, &steps, sizeof(uint32_t));
	if (ret) {
		tzdev_smc_channel_release(ch);
		return ret;
	}
		...
		struct list_head *cleaned_head)
{
	...
	list_for_each_entry_safe(profiler_buf, tmp, head, list) {
		tzio_buf = profiler_get_tzio_circ(profiler_buf);
