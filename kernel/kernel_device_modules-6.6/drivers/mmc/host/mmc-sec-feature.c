...
static void sec_android_mmc_update_mmc_queue(void *data, struct mmc_card *card, struct mmc_queue *mq)
{
	...
	if (!host->max_busy_timeout)
		blk_queue_max_discard_sectors(mq->queue, UINT_MAX);
}

void sd_sec_register_vendor_hooks(void)
{
	register_trace_android_vh_mmc_update_mmc_queue(sec_android_mmc_update_mmc_queue, NULL);
}
