...
#define AF_DRVNAME "[DW9818AF D/D]"
...
static int create_actuator_sysfs(void);
...
static int initAF(void)
{
	...
	if (!IMGSENSOR_GET_SAC_VALUE_BY_SENSOR_IDX(0, &sac_mode, &sac_time)) {
		ret |= -1;
		LOG_ERR("[%s] DW9818 : failed to get sac value\n", __func__);
	}
...
static ssize_t sac_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	LOG_INF("SAC Mode    , addr:%#x, data:%#x\n", REG_MODE, read_data(REG_MODE));
	LOG_INF("SAC Prescale, addr:%#x, data:%#x\n", REG_PRESCALE, read_data(REG_PRESCALE));
	LOG_INF("SAC time    , addr:%#x, data:%#x\n", REG_SACTIMING, read_data(REG_SACTIMING));

	return sprintf(buf, "SAC(addr:data)->[0x%X:0x%X, 0x%X:0x%X, 0x%X:0x%X]\n",
			REG_MODE, read_data(REG_MODE),
			REG_PRESCALE, read_data(REG_PRESCALE),
			REG_SACTIMING, read_data(REG_SACTIMING));
}
static DEVICE_ATTR_RO(sac_data);
...
static int create_actuator_sysfs(void)
{
	...
	if (IS_ERR_OR_NULL(camera_class)) {
		LOG_ERR("actuator_sysfs: error, camera class not exist\n");
		return -ENODEV;
	}

	if (actuator_dev == NULL)
		actuator_dev = device_create(camera_class, NULL, 0, NULL, "actuator");

	if (IS_ERR(actuator_dev)) {
		LOG_ERR("actuator_sysfs: failed to create device(actuator)\n");
		return -ENODEV;
	}
#else
	LOG_ERR("actuator_sysfs: failed to build sysfs\n");
#endif

	err = device_create_file(actuator_dev, &dev_attr_sac_data);
	if (unlikely(err < 0)) {
		if (err == -EEXIST) {
			LOG_INF("actuator_sysfs file already exists\n");
		} else {
			LOG_ERR("actuator_sysfs: failed to create device file, %s[err:%d]\n",
					dev_attr_sac_data.attr.name, err);
		}
	}
