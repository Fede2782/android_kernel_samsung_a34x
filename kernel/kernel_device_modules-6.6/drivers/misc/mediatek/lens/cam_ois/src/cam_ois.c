...
int cam_ois_mcu_update(void)
{
	...
	if (mcu_info.pinctrl != NULL && i2c_client != NULL) {
		...
		LOG_ERR("fw_updated %d, pinctrl 0x%p, i2c 0x%p", mcu_info.is_updated, (void *)(mcu_info.pinctrl), (void *)i2c_client);
	}
	return 0;
}
...
{
	LOG_DBG("pinctrl: 0x%p, mcu boot: 0x%p, 0x%p, mcu nRST: 0x%p, 0x%p, mcu VDD: 0x%p, 0x%p, ois VDD: 0x%p, 0x%p",
		(void *)mcu_info->pinctrl, (void *)mcu_info->mcu_boot_high, (void *)mcu_info->mcu_boot_low,
		(void *)mcu_info->mcu_nrst_high, (void *)mcu_info->mcu_nrst_low, (void *)mcu_info->mcu_vdd_high,
		(void *)mcu_info->mcu_vdd_low, (void *)mcu_info->ois_vdd_high, (void *)mcu_info->ois_vdd_low);
}
