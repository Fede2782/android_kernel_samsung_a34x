...
static inline void Prepare_Enable_ccf_clock(enum ISP_DEV_NODE_ENUM module)
{
	...
	LOG_NOTICE("pm_runtime_get_sync: ISP_CAMSYS_CONFIG_IDX");
	if (ret < 0)
	...
	if (module == ISP_CAM_A_IDX) {
		...
		LOG_NOTICE("pm_runtime_get_sync: ISP_CAM_A_IDX");
		if (ret < 0)
	...
	if (module == ISP_CAM_B_IDX) {
		...
		LOG_NOTICE("pm_runtime_get_sync: ISP_CAM_B_IDX");
		if (ret < 0)
	...
	if (IS_3RAW_PLATFORM(g_platform_id)) {
		if (module == ISP_CAM_C_IDX) {
			...
			LOG_NOTICE("pm_runtime_get_sync: ISP_CAM_C_IDX");
			if (ret < 0)
				LOG_NOTICE("cannot pm runtime get ISP_CAM_C_IDX mtcmos\n");
		}
	}
	LOG_NOTICE("clk_prepare_enable +++");
...
static inline void Disable_Unprepare_ccf_clock(enum ISP_DEV_NODE_ENUM module)
{
	...
	LOG_NOTICE("clk_disable_unprepare +++");
	...
	if (module == ISP_CAM_C_IDX) {
		if (IS_3RAW_PLATFORM(g_platform_id)) {
			...
			LOG_NOTICE("pm_runtime_put_sync: ISP_CAM_C_IDX");
			if (ret < 0)
	...
	if (module == ISP_CAM_B_IDX) {
		...
		LOG_NOTICE("pm_runtime_put_sync: ISP_CAM_B_IDX");
		if (ret < 0)
	...
	if (((IrqStatus & SOF_INT_ST) && isStagger == 0) ||
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (sec_on)
				...
				IRQ_LOG_KEEPER(
				module, m_CurrentPPB, _LOG_INF,
				"%s,%s,CAM_%c P1_SOF_%d_%d(0x%08x_0x%08x,0x%08x_0x%08x,0x%08x,0x%08x,0x%x/0x%x),int_us:%d,FBC:0x%08x,cq:0x%08x_0x%08x 0x%08x_0x%08x 0x%08x_0x%08x,Don(0x%08x_0x%08x 0x%08x_0x%08x,0x%08x_0x%08x 0x%08x_0x%08x),DMA(0x%x_0x%x,0x%x_0x%x,0x%x_0x%x,0x%x_0x%x,0x%x_0x%x,0x%x_0x%x),CTL_EN(0x%x_0x%x 0x%x_0x%x 0x%x_0x%x),CTL_EN2(0x%x_0x%x 0x%x_0x%x 0x%x_0x%x) LMV(0x%x,0x%x_0x%x)\n",
				...
				(unsigned int)ISP_RD32(
					CAM_REG_CTL_EN2(
						ISP_CAM_C_INNER_IDX)),
				(unsigned int)ISP_RD32(
					CAM_REG_LMVO_BASE_ADDR(
						ISP_CAM_A_IDX)),
				(unsigned int)ISP_RD32(
					CAM_REG_CTL_DMA_EN(ISP_CAM_A_IDX)) & 0x10000,
				(unsigned int)ISP_RD32(
					CAM_REG_CTL_EN2(ISP_CAM_A_IDX)) & 0x8000);
			}
