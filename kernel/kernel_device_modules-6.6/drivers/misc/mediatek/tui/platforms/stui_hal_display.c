...
static int disp_tui_prepare(void *disp_dev_data, bool tui_en)
{
	...
	if (tui_en) {
		/* TODO:
		 * Prepare display controller's SFR registers to start TUI here
#if IS_ENABLED(CONFIG_PM_RUNTIME)
		ret = pm_runtime_get_sync(disp_dev_data->dev);
		if (ret < 0) {
			pr_err("%s, pm_runtime_get_sync error ret = %d", __func__, ret);
			return ret;
		}
#endif
		 */
		...
		/* TODO:
		 * Restore display controller's SFR registers here
#if IS_ENABLED(CONFIG_PM_RUNTIME)
		ret = pm_runtime_put_sync(disp_dev_data->dev);
		if (ret < 0) {
			pr_err("%s, pm_runtime_put_sync error ret = %d", __func__, ret);
			return ret;
		}
#endif
		 */
...
int stui_alloc_video_space(struct tui_hw_buffer *buffer)
{
	...
	uint32_t width = DISP_GetScreenWidth();
	uint32_t height = DISP_GetScreenHeight();

	framebuf_size = (width * height * (DEFAULT_BPP / 8));
	/* TODO: set different alignment, if needed */
	...
	ret = trusted_mem_api_query_pa(0, 0, 0, 0, &sec_handle, 0, 0, 0, &phys_addr);
	if (ret != 0) {
