...
static struct vcp_work_struct vcp_logger_notify_work;
...
enum vcp_core_id vcp_logger_core_id = VCP_ID;
uint32_t vcp_logger_out_ipi_pin = IPI_OUT_LOGGER_CTRL_0;
uint32_t vcp_logger_in_ipi_pin = IPI_IN_LOGGER_CTRL_0;
struct mtk_ipi_device *vcp_logger_ipi_dev = &vcp_ipidev;
...
static size_t vcp_A_get_last_log(size_t b_len)
{
	...
	/* VCP keep awake */
	vcp_awake_flag = 0;
	if (vcp_awake_lock((void *)vcp_logger_core_id) == -1) {
		vcp_awake_flag = -1;
...
exit:
	/*VCP release awake */
	if (vcp_awake_flag == 0) {
		if (vcp_awake_unlock((void *)vcp_logger_core_id) == -1)
...
ssize_t vcp_A_log_read(char __user *data, size_t len)
{
	...
	if (strcmp(current->comm, "mobile_log_d.rd") != 0) {
		...
		if (!log_ctl_debug) {
			...
			if (is_vcp_ready_by_coreid(vcp_logger_core_id)) {
				msg.cmd = VCP_LOGGER_IPI_FLUSH;
				ret = mtk_ipi_send(vcp_logger_ipi_dev, vcp_logger_out_ipi_pin,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = mtk_ipi_send(vcp_logger_ipi_dev, vcp_logger_out_ipi_pin, 0, &msg,
...
static int vcp_logger_init_handler(struct VCP_LOG_INFO *log_info)
{
	...
	vcp_logger_notify_work.id = vcp_logger_core_id;
#if VCP_LOGGER_ENABLE
	vcp_logger_notify_work.flags = 0;
	vcp_schedule_logger_work(&vcp_logger_notify_work);
#endif
...
static void vcp_logger_notify_ws(struct work_struct *ws)
{
	...
	vcp_ipi_id = vcp_logger_out_ipi_pin;
	...
	do {
		if (is_vcp_ready_by_coreid(vcp_logger_core_id)) {
			...
			ret = mtk_ipi_send(vcp_logger_ipi_dev, vcp_ipi_id, 0, &msg,
					   sizeof(msg)/MBOX_SLOT_SIZE, 0);
...
int vcp_logger_init(phys_addr_t start, phys_addr_t limit)
{
	...
	vcp_logger_core_id = VCP_ID;
	vcp_logger_out_ipi_pin = IPI_OUT_LOGGER_CTRL_0;
	vcp_logger_in_ipi_pin = IPI_IN_LOGGER_CTRL_0;
	vcp_logger_ipi_dev = &vcp_ipidev;

	/*init work queue*/
	INIT_WORK(&vcp_logger_notify_work.work, vcp_logger_notify_ws);
	...
	mtk_ipi_register(vcp_logger_ipi_dev, vcp_logger_in_ipi_pin,
