		...
		struct device *dev)
{
	...
	struct syna_hw_pwr_data *pwr = &hw_if->bdata_pwr;
	struct syna_hw_rst_data *rst = &hw_if->bdata_rst;
	struct syna_board_data *bdata = &hw_if->bdata;
	...
	prop = of_find_property(np, "support_ear_detect_mode", NULL);
	if (prop && prop->length) {
		value = of_property_read_bool(np, "support_ear_detect_mode");
		bdata->support_ear_detect = value;
	} else {
		bdata->support_ear_detect = 0;
	}
...
// 'int (*)(struct i2c_client *)' with an expression of type 'int (struct i2c_client *, const struct i2c_device_id *)' 
