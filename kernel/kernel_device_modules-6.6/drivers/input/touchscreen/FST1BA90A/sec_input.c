...
void sec_input_support_feature_parse_dt(struct device *dev)
{
	...
	pdata->dev = dev;
	pdata->regulator_boot_on = of_property_read_bool(np, "sec,regulator-boot-on");
	pdata->support_dex = of_property_read_bool(np, "support-dex-mode");
	pdata->support_fod = of_property_read_bool(np, "support-fod");
	pdata->support_fod_lp_mode = of_property_read_bool(np, "support-fod-lp-mode");
	pdata->enable_settings_aot = of_property_read_bool(np, "enable-settings-aot");
	pdata->support_refresh_rate_mode = of_property_read_bool(np, "support-refresh-rate-mode");
	pdata->support_vrr = of_property_read_bool(np, "support-vrr");
	pdata->support_ear_detect = of_property_read_bool(np, "support-ear-detect-mode");
	pdata->support_open_short_test = of_property_read_bool(np, "support-open-short-test");
	pdata->support_mis_calibration_test = of_property_read_bool(np, "support-mis-calibration-test");
	pdata->support_wireless_tx = of_property_read_bool(np, "support-wireless-tx");
	pdata->support_input_monitor = of_property_read_bool(np, "support-input-monitor");
	pdata->disable_vsync_scan = of_property_read_bool(np, "disable-vsync-scan");
	pdata->sense_off_when_cover_closed = of_property_read_bool(np, "sense-off-when-cover-closed");
	pdata->not_support_temp_noti = of_property_read_bool(np, "not-support-temp-noti");
	pdata->support_vbus_notifier = of_property_read_bool(np, "support-vbus-notifier");
	pdata->support_gesture_uevent = of_property_read_bool(np, "support-gesture-uevent");
	pdata->support_always_on = of_property_read_bool(np, "support-always-on");

	if (of_property_read_u32(np, "support-rawdata-map-num", &pdata->support_rawdata_map_num) < 0)
		pdata->support_rawdata_map_num = 0;

	if (of_property_read_u32(np, "sec,support-sensor-hall", &pdata->support_sensor_hall) < 0)
		pdata->support_sensor_hall = 0;
	pdata->support_lightsensor_detect = of_property_read_bool(np, "support-lightsensor-detect");

	pdata->prox_lp_scan_enabled = of_property_read_bool(np, "sec,prox-lp-scan-enabled");
	input_info(true, dev, "%s: Prox LP Scan enabled %s\n",
				__func__, pdata->prox_lp_scan_enabled ? "ON" : "OFF");

	pdata->enable_sysinput_enabled = of_property_read_bool(np, "sec,enable-sysinput-enabled");
	input_info(true, dev, "%s: Sysinput enabled %s\n",
				__func__, pdata->enable_sysinput_enabled ? "ON" : "OFF");

	pdata->support_rawdata = of_property_read_bool(np, "sec,support-rawdata");
	input_info(true, dev, "%s: report rawdata %s\n",
				__func__, pdata->support_rawdata ? "ON" : "OFF");
	pdata->support_rawdata_motion_aivf = of_property_read_bool(np, "sec,support-rawdata-motion-aivf");
	input_info(true, dev, "%s: motion aivf %s\n",
				__func__, pdata->support_rawdata_motion_aivf ? "ON" : "OFF");
	pdata->support_rawdata_motion_palm = of_property_read_bool(np, "sec,support-rawdata-motion-palm");
...
int sec_input_parse_dt(struct device *dev)
{
	...
	pdata->dev = dev;
	pdata->chip_on_board = of_property_read_bool(np, "chip-on-board");
	 ...
	 */
	count = of_property_count_u32_elems(np, "sec,bitmask-unload");
	if (lcd_type != 0 && count > 1 && count < 10 &&
			(!of_property_read_u32_array(np, "sec,bitmask-unload", lcd_bitmask, count))) {
	...
	pdata->gpio_spi_cs = of_get_named_gpio(np, "sec,gpio-spi-cs", 0);
	if (gpio_is_valid(pdata->gpio_spi_cs)) {
		ret = gpio_request(pdata->gpio_spi_cs, "tsp,gpio-spi-cs");
	...
	if (of_property_read_u32_array(np, "sec,max-coords", coords, 2)) {
	...
	count = of_property_count_strings(np, "sec,firmware-name");
	if (count <= 0) {
		pdata->firmware_name = NULL;
	} else {
		u8 lcd_id_num = of_property_count_u32_elems(np, "sec,select-lcdid");

		if ((lcd_id_num != count) || (lcd_id_num <= 0)) {
			of_property_read_string_index(np, "sec,firmware-name", 0, &pdata->firmware_name);
		} else {
			u32 *lcd_id_t;
			u32 lcd_id_mask;
			...
			of_property_read_u32_array(np, "sec,select-lcdid", lcd_id_t, lcd_id_num);
			...
			for (i = 0; i < lcd_id_num; i++) {
				if (((lcd_id_t[i] & lcd_id_mask) == (lcd_type & lcd_id_mask)) || (i == (lcd_id_num - 1))) {
					of_property_read_string_index(np, "sec,firmware-name", i, &pdata->firmware_name);
					break;
				}
			}
	...
	pdata->not_support_vdd = of_property_read_bool(np, "not-support-vdd");
	if (!pdata->not_support_vdd) {
		const char *avdd_name, *dvdd_name;

		pdata->not_support_io_ldo = of_property_read_bool(np, "not-support-io-ldo");
		if (!pdata->not_support_io_ldo) {
			if (of_property_read_string(np, "sec,dvdd-name", &dvdd_name) < 0)
		...
		if (of_property_read_string(np, "sec,avdd-name", &avdd_name) < 0)
	...
	if (of_property_read_u32(np, "sec,dump-ic-ver", &pdata->dump_ic_ver) < 0)
		pdata->dump_ic_ver = 0;
...
#if IS_ENABLED(CONFIG_INPUT_SEC_SECURE_TOUCH)
	of_property_read_u32(np, "sec,ss-touch-num", &pdata->ss_touch_num);
	...
	pdata->work_queue_probe_enabled = of_property_read_bool(np, "work-queue-probe-enabled");
	if (pdata->work_queue_probe_enabled) {
	...
	if (of_property_read_u32(np, "sec,probe-queue-delay", &pdata->work_queue_probe_delay)) {
...
int sec_input_multi_device_parse_dt(struct device *dev)
{
	...
	if (of_property_read_u32(np, "sec,multi-device-count", &device_count) < 0)
...
void sec_tclm_parse_dt(struct device *dev, struct sec_tclm_data *tdata)
{
	...
	if (of_property_read_u32(np, "sec,tclm-level", &tdata->tclm_level) < 0) {
		tdata->tclm_level = 0;
		input_err(true, dev, "%s: Failed to get tclm_level property\n", __func__);
	}

	if (of_property_read_u32(np, "sec,afe-base", &tdata->afe_base) < 0) {
		tdata->afe_base = 0;
		input_err(true, dev, "%s: Failed to get afe_base property\n", __func__);
	}

	tdata->support_tclm_test = of_property_read_bool(np, "support-tclm-test");
...
int sec_input_check_fw_name_incell(struct device *dev, const char **firmware_name, const char **firmware_name_2nd)
{
	...
	vendor_chk_gpio = of_get_named_gpio(np, "sec,vendor-check-gpio", 0);
	if (gpio_is_valid(vendor_chk_gpio)) {
		int vendor_chk_en_val = 0;
		int vendor_chk_gpio_val = gpio_get_value(vendor_chk_gpio);

		ret = of_property_read_u32(np, "sec,vendor-check-enable-value", &vendor_chk_en_val);
		if (ret < 0) {
	...
	fw_name_cnt = of_property_count_strings(np, "sec,fw-name");
	...
	ret = of_property_read_string_index(np, "sec,fw-name", fw_sel_idx, firmware_name);
	...
	/* only for novatek */
	if (of_property_count_strings(np, "sec,fw-name-2nd") > 0) {
