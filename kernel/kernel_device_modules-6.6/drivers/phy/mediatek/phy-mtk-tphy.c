...
static void phy_set_default_value(struct mtk_phy_instance *instance)
{
	instance->eye_src = -1;
	instance->eye_vrt = -1;
	instance->eye_term = -1;
	instance->intr = -1;
	instance->discth = -1;
	instance->pre_emphasis = -1;
	instance->rx_sqth = -1;
	instance->rev4 = -1;
	instance->rev6 = -1;
	instance->eye_src_host = -1;
	instance->eye_vrt_host = -1;
	instance->eye_term_host = -1;
	instance->rev6_host = -1;
	instance->discth_host = -1;
	instance->pll_bw = -1;
	instance->bgr_div = -1;
	instance->fsrxlvl = -1;
	instance->eq_leq_shift = -1;
}
				...
				struct mtk_phy_instance *instance)
{
	...
	if (!instance->phy_property_init_done) {
		phy_set_default_value(instance);
		...
		if (!device_property_read_u32(dev, "mediatek,eye-term", &val))
			instance->eye_term = val;
		if (!device_property_read_u32(dev, "mediatek,intr", &val))
			instance->intr = val;
		if (!device_property_read_u32(dev, "mediatek,discth", &val))
			instance->discth = val;
		if (!device_property_read_u32(dev, "mediatek,pre-emphasis", &val))
			instance->pre_emphasis = val;
		if (!device_property_read_u32(dev, "mediatek,rx-sqth", &val))
			instance->rx_sqth = val;
		if (!device_property_read_u32(dev, "mediatek,rev4", &val))
			instance->rev4 = val;
		if (!device_property_read_u32(dev, "mediatek,rev6", &val))
			instance->rev6 = val;
		if (!device_property_read_u32(dev, "mediatek,eye-src-host", &val))
			instance->eye_src_host = val;
		if (!device_property_read_u32(dev, "mediatek,eye-vrt-host", &val))
			instance->eye_vrt_host = val;
		if (!device_property_read_u32(dev, "mediatek,eye-term-host", &val))
			instance->eye_term_host = val;
		if (!device_property_read_u32(dev, "mediatek,rev6-host", &val))
			instance->rev6_host = val;
		if (!device_property_read_u32(dev, "mediatek,disc-host", &val))
			instance->discth_host = val;
		if (!device_property_read_u32(dev, "mediatek,pll-bw", &val))
			instance->pll_bw = val;
		if (!device_property_read_u32(dev, "mediatek,bgr-div", &val))
			instance->bgr_div = val;
		if (!device_property_read_u32(dev, "mediatek,fsrxlvl", &val))
			instance->fsrxlvl = val;
		if (!device_property_read_u32(dev, "mediatek,eq-leq-shift", &val))
			instance->eq_leq_shift = val;
		instance->usb_special_phy_settings = device_property_read_bool(dev,
					"mediatek,usb-special-phy-settings");
		instance->phy_property_init_done = true;
	}
	dev_info(dev, "bc12:%d, src:%d, vrt:%d, term:%d, intr:%d, disc:%d\n",
		instance->bc12_en, instance->eye_src,
		instance->eye_vrt, instance->eye_term,
		instance->intr, instance->discth);
	dev_info(dev, "pre-emp:%d\n", instance->pre_emphasis);
	dev_info(dev, "rx_sqth:%d\n", instance->rx_sqth);
	dev_info(dev, "rev4:%d, rev6:%d\n", instance->rev4, instance->rev6);
	dev_info(dev, "pll-bw:%d, bgr-div:%d\n", instance->pll_bw, instance->bgr_div);
	dev_info(dev, "fsrxlvl:%d, eq-leq-shift:%d\n", instance->fsrxlvl, instance->eq_leq_shift);
	dev_info(dev, "usb-special-phy-settings:%d\n", instance->usb_special_phy_settings);
}
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	if ((tphy->pdata->version < MTK_PHY_V3) && (instance->eye_src >= 0))
		mtk_phy_update_field(com + U3P_USBPHYACR5, PA5_RG_U2_HSTX_SRCTRL,
				     instance->eye_src);
	...
	if (instance->discth >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR6, PA6_RG_U2_DISCTH,
				     instance->discth);

	if (instance->pre_emphasis >= 0)
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	if (instance->eye_src_host >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR5, PA5_RG_U2_HSTX_SRCTRL,
				instance->eye_src_host);

	if (instance->eye_vrt_host >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR1, PA1_RG_VRT_SEL,
				instance->eye_vrt_host);

	if (instance->eye_term_host >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR1, PA1_RG_TERM_SEL,
				instance->eye_term_host);

	if (instance->rev6_host >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR6, PA6_RG_U2_PHY_REV6,
				instance->rev6_host);

	if (instance->discth_host >= 0)
		mtk_phy_update_field(com + U3P_USBPHYACR6, PA6_RG_U2_DISCTH,
