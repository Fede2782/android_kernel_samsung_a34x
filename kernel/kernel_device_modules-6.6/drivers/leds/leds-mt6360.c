...
#define MT6360_STRBTIMEOUT_DEFAULT		800000 /* 800ms */
...
#define MT6360_FLASH_DEFAULT_CUR		1300 /*in mA*/
#define MT6360_FACTORY_DEFAULT_CUR		200 /*in mA*/
			   ...
			   struct led_classdev_flash *fled_cdev, u32 brightness)
{
	...
	dev_info(led_cdev->dev, "[%s][D/D]: id[%d], brightness - strobe current(%u mA)\n", __func__, id, brightness/1000);
				      ...
				      enum led_brightness brightness)
{
	...
	dev_info(led_cdev->dev, "[%s][D/D]: id [%d], brightness - torch current[level(%d)->(%d mA)]\n",
			__func__, id, brightness, 25 + ((25 * brightness) / 2));
out_bright_set:
	return ret;
}
				     ...
				     struct v4l2_flash *v4l2_flash, bool enable)
{
	...
	if (enable == false) {
		/* Torch Off*/
		mt6360_fled_brightness_set(led_cdev, 0);
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	flcdev = mt6360_flash_class[0]; /* we use single channel: ch1[0], ch2[1]) */
	if (flcdev == NULL) {
		pr_info("Get flcdev failed\n");
		return -EINVAL;
	}
	...
	if (value <= 0) {
		...
		/* Factory Torch*/
		pr_info("%s: torch current [%d]\n", __func__, g_fled_data->torch_current);
		current_level = g_fled_data->torch_current;
		mode = MT6360_ENABLE_TORCH;
	} else if (value == 200) {
		/* Factory Flash */
		pr_info("%s: factory_current [%d]\n", __func__, g_fled_data->factory_current);
		mode = MT6360_ENABLE_FLASH;
	...
	if (mode == MT6360_ENABLE_TORCH) {
		mt6360_fled_brightness_set(lcdev, current_level);
	} else if (mode == MT6360_ENABLE_FLASH) {
		mt6360_fled_strobe_brightness_set(flcdev, current_level * 1000); /*current val passed in uA*/
		mt6360_fled_strobe_timeout_set(flcdev, MT6360_STRBTIMEOUT_DEFAULT);
		mt6360_fled_strobe_set(flcdev, true);
	} else {
		mt6360_fled_brightness_set(lcdev, 0);
		mt6360_fled_strobe_get(flcdev, &flash_status);
		if (flash_status)
			mt6360_fled_strobe_set(flcdev, false);
	}

	pr_info("%s: input value - %d, mode:%d\n", __func__, current_level, mode);
				...
				struct mt6360_led_info *mli)
{
	...
	for_each_available_child_of_node(fled_np, child) {
		...
		if (!strcmp(mtfled_cdev->fl_cdev.led_cdev.name, "mt6360_flash_ch1")) {
			ret = of_property_read_u32(child, "torch_current", &g_fled_data->torch_current);
			if (ret) {
				dev_warn(dev, "torch_current property missing\n");
				g_fled_data->torch_current = 1;
			}

			ret = of_property_read_u32(child, "flash_current", &g_fled_data->flash_current);
			if (ret) {
				dev_warn(dev, "flash_current property missing\n");
				g_fled_data->flash_current = MT6360_FLASH_DEFAULT_CUR;
			}

			ret = of_property_read_u32(child, "factory_current", &g_fled_data->factory_current);
			if (ret) {
				dev_warn(dev, "factory_current property missing\n");
				g_fled_data->factory_current = MT6360_FACTORY_DEFAULT_CUR;
			}

			ret = of_property_read_u32_array(child, "flashlight_current",
					g_fled_data->flashlight_current, MT6360_FLASH_LIGHT_MAX);
			if (ret < 0) {
				pr_err("%s : could not find flashlight_current\n", __func__);

				//default setting
				g_fled_data->flashlight_current[0] = 1;
				g_fled_data->flashlight_current[1] = 2;
				g_fled_data->flashlight_current[2] = 3;
				g_fled_data->flashlight_current[3] = 4;
				g_fled_data->flashlight_current[4] = 5;
			}
		}
#if IS_ENABLED(CONFIG_MTK_FLASHLIGHT)
...
static int mt6360_led_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		g_fled_data = devm_kzalloc(&pdev->dev, sizeof(struct mt6360_fled_data), GFP_KERNEL);
		if (!g_fled_data)
			return -ENOMEM;
