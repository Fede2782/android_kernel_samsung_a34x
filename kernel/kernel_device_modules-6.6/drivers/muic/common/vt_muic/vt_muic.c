		...
		unsigned long action, void *data)
{
	...
	case PDIC_NOTIFY_ID_WATER:
		if (pnoti->sub1)
			vt_muic.water_state = 1;
		else
			vt_muic.water_state = 0;
		break;
...
int vt_muic_get_afc_disable(void)
{
	...
	if (is_lpcharge_pdic_param() && vt_muic.water_state) {
		ret = 1;
		goto out;
	}
...
void vt_muic_set_water_state(int state, bool en)
{
	struct vt_muic_ic_data *ic_data = vt_muic.ic_data;

	if (ic_data && ic_data->m_ops.set_cc_high_state)
		ic_data->m_ops.set_cc_high_state(ic_data->mdata, state, en);
}
EXPORT_SYMBOL(vt_muic_set_water_state);
...
static int vt_muic_get_hiccup_cb(void *data)
{
	struct vt_muic_ic_data *ic_data = vt_muic.ic_data;
	int ret = 0;

	if (ic_data && ic_data->m_ops.get_hiccup_mode)
		ret = ic_data->m_ops.get_hiccup_mode(ic_data->mdata);

	return ret;
}

static int vt_muic_get_hiccup_mode_cb(void)
{
	struct vt_muic_ic_data *ic_data = vt_muic.ic_data;
	int ret = 0;
...
static void vt_muic_fill_muic_sysfs_cb(struct muic_platform_data *pdata)
{
	...
	pdata->sysfs_cb.get_hiccup = vt_muic_get_hiccup_cb;
#endif
}
...
static void vt_muic_fill_muic_cb(struct muic_platform_data *pdata)
{
	...
	pdata->muic_get_hiccup_mode_cb = vt_muic_get_hiccup_mode_cb;
#endif
}
#endif
