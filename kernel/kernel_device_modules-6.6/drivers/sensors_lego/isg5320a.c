...
static u8 isg5320a_get_grip_state(u8 intr_msg)
{
	return (intr_msg & (1 << ISG5320A_PROX_STATE)) ? CLOSE : FAR;
}
...
static void check_irq_error(struct isg5320a_data *data, u8 irq_state, bool is_irq_func, bool is_enable_func)
{
	...
	if (data->is_irq_active && data->check_abnormal_working == false) {
		state = isg5320a_get_grip_state(irq_state);
...
static void isg5320a_calculate_irq_count_and_max_diff(struct isg5320a_data *data, int state)
{
	if (data->abnormal_mode && state == CLOSE) {
...
static void isg5320a_update_grip_state(struct isg5320a_data *data, int state)
{
	switch (data->state) {
	case FAR:
		if (state == CLOSE) {
...
static void isg5320a_send_event_to_hal(struct isg5320a_data *data, int state)
{
	u8 grip_data = (data->state == CLOSE) ? GRIP_CLOSE : GRIP_FAR;

	input_report_rel(data->input_dev, REL_MISC, grip_data);
	input_report_rel(data->input_dev, REL_X, data->is_unknown_mode);
	input_sync(data->input_dev);
}

static void isg5320a_handle_unknown_mode_event(struct isg5320a_data *data)
{
	if (data->is_unknown_mode != UNKNOWN_ON || !data->motion) {
		GRIP_INFO("unknown mode off and no motion detected\n");
		return;
	}

	if (data->state == CLOSE) {
		data->first_working = true;
	} else if (data->state == FAR && data->first_working) {
...
static void isg5320a_send_event(struct isg5320a_data *data, u8 intr_msg)
{
	u8 state;
	...
	state = isg5320a_get_grip_state(intr_msg);

	isg5320a_update_grip_state(data, state);
	isg5320a_calculate_irq_count_and_max_diff(data, state);
	isg5320a_handle_unknown_mode_event(data);
	isg5320a_send_event_to_hal(data, state);
}
...
static irqreturn_t isg5320a_irq_thread(int irq, void *ptr)
{
	...
	isg5320a_send_event(data, intr_msg);
...
static void isg5320a_set_enable(struct isg5320a_data *data, int enable)
{
	...
	if (enable == ON) {
		...
		} else if (isg5320a_get_grip_state(state)) {
			data->state = CLOSE;
			input_report_rel(data->input_dev, REL_MISC, 1);
			input_report_rel(data->input_dev, REL_X, data->is_unknown_mode);
		} else {
