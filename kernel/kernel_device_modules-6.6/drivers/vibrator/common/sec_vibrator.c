...
__visible_for_testing ssize_t freq_default_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sec_vibrator_drvdata *ddata;
	...
	ddata = dev_get_drvdata(dev);
	if (!ddata->vib_ops->get_freq_default)
		return -ENOSYS;

	return ddata->vib_ops->get_freq_default(ddata->dev, buf);
}
...
static DEVICE_ATTR_RO(freq_default);
