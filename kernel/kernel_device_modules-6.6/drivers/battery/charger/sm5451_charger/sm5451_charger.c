	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (atomic_read(&sm5451->shutdown_cnt) > 0) {
		dev_info(sm5451->dev, "%s: DC already shutdown\n", __func__);
		return -EINVAL;
	}
...
#endif
{
	...
	atomic_set(&sm5451->shutdown_cnt, 0);
...
#else
static void sm5451_charger_remove(struct i2c_client *i2c)
#endif
...
static void sm5451_charger_shutdown(struct i2c_client *i2c)
{
	...
	atomic_inc(&sm5451->shutdown_cnt);
	sm5451_stop_charging(sm5451);
	sm5451_reverse_boost_enable(sm5451, false);
