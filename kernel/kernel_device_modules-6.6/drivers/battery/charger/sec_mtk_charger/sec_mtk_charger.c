...
static void mtk_charger_ob_mode(struct mtk_charger_data *charger, bool enable)
{
	...
	if (enable) {
		...
		pr_info("[DEBUG]%s: set buck on\n", __func__);
		charger_dev_enable_powerpath(charger->chg_dev, true);
		if (charger->pdata->mt6360_charger == true)
			charger_dev_set_constant_voltage(charger->chg_dev, (4200 * 1000));
		else
		...
		/* Set Input current limit to max (mt6360_charger:3250mA, mt6375_charger:3225mA) */
...
#endif
		ret = of_property_read_u32(np, "charger,mt6360_charger",
									&pdata->mt6360_charger);
		if (ret) {
			pr_info("%s: mt6360_charger is empty\n", __func__);
			pdata->mt6360_charger = 0;
		}
		pr_info("%s: mt6360_charger %d\n", __func__, pdata->mt6360_charger);
	}
