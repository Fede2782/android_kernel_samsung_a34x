...
__visible_for_testing void sec_bat_divide_limiter_current(struct sec_battery_info *battery, int limiter_current)
{
	...
	if (battery->pdata->sub_fto && limiter_current >= battery->pdata->sub_fto_current_thresh)
		psy_set_property(battery->pdata->sub_limiter_name, POWER_SUPPLY_EXT_PROP_CHG_MODE, 3);
	else if (battery->pdata->sub_fto && limiter_current < battery->pdata->sub_fto_current_thresh)
		psy_set_property(battery->pdata->sub_limiter_name, POWER_SUPPLY_EXT_PROP_CHG_MODE, 0);
...
{
#if IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	pr_info("%s: charge_m(%d), charge_s(%d), charge_t(%d)\n", __func__,
	...
	psy_set_property(battery->pdata->main_limiter_name,
		POWER_SUPPLY_EXT_PROP_FASTCHG_LIMIT_CURRENT, battery->main_current);
	psy_set_property(battery->pdata->sub_limiter_name,
		POWER_SUPPLY_EXT_PROP_FASTCHG_LIMIT_CURRENT, battery->sub_current);
#if IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	psy_set_property(battery->pdata->third_limiter_name,
		POWER_SUPPLY_EXT_PROP_FASTCHG_LIMIT_CURRENT, battery->third_current);
#endif
}
...
__visible_for_testing int set_vlim_voltage(void *data, int voltage)
{
	...
	psy_set_property(battery->pdata->sub_limiter_name,
		POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE, voltage);
#if IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	psy_set_property(battery->pdata->third_limiter_name,
		POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE, voltage);
#endif
...
static bool sec_bat_check_by_gpio(struct sec_battery_info *battery)
{
	...
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_EXT_PROP_DUAL_BAT_DET, SEC_DUAL_BATTERY_MAIN, main_det);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_EXT_PROP_DUAL_BAT_DET, SEC_DUAL_BATTERY_SUB, sub_det);
...
static void sec_bat_powerpath_check(struct sec_battery_info *battery, int m_health, int s_health)
{
	...
	/* get main, sub limiter's supplement status */
	psy_get_property(battery->pdata->main_limiter_name,
		POWER_SUPPLY_EXT_PROP_SUPLLEMENT_MODE, 0, m_supplement_status);
	psy_get_property(battery->pdata->sub_limiter_name,
		POWER_SUPPLY_EXT_PROP_SUPLLEMENT_MODE, 0, s_supplement_status);
...
static void sec_bat_limiter_check(struct sec_battery_info *battery)
{
	...
	if (m_value.intval != POWER_SUPPLY_HEALTH_GOOD) {
		...
		psy_set_property(battery->pdata->main_limiter_name,
			POWER_SUPPLY_EXT_PROP_DISCHG_LIMIT_CURRENT, 1000);
		psy_set_property(battery->pdata->main_limiter_name,
			POWER_SUPPLY_EXT_PROP_DISCHG_MODE, 0);
		...
		psy_set_property(battery->pdata->main_limiter_name,
			POWER_SUPPLY_EXT_PROP_POWERMETER_ENABLE, 1);
		msleep(100);
...
#if IS_ENABLED(CONFIG_DUAL_BATTERY) || IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	psy_set_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_EXT_PROP_FULL_CONDITION, battery->pdata->age_step);
...
#else
		psy_set_property(battery->pdata->dual_battery_name, POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE,
			battery->pdata->chg_float_voltage - battery->pdata->limiter_aging_float_offset);
#endif
#if IS_ENABLED(CONFIG_DIRECT_CHARGING)
...
void sec_bat_aging_check_dual_battery_individual_fv(struct sec_battery_info *battery)
{
	...
	psy_set_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_EXT_PROP_MAIN_LIMITER_CHG_VOLTAGE, fv - offset);
	...
	psy_set_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_EXT_PROP_SUB_LIMITER_CHG_VOLTAGE, fv - offset);
}
...
int sec_bat_dual_battery_vbat(struct sec_battery_info *battery, int battery_type)
{
	...
	case SEC_DUAL_BATTERY_MAIN:
		...
		psy_get_property(pdata->dual_battery_name, POWER_SUPPLY_PROP_VOLTAGE_NOW, battery_type, ret);
...
{
#if IS_ENABLED(CONFIG_DUAL_SBP)
	/* get main/sub voltage now */
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_VOLTAGE_NOW, SEC_DUAL_BATTERY_MAIN, battery->voltage_now_main);
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_VOLTAGE_NOW, SEC_DUAL_BATTERY_SUB, battery->voltage_now_sub);
#else
	/* get main/sub voltage now */
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, SEC_DUAL_BATTERY_MAIN, battery->voltage_now_main);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, SEC_DUAL_BATTERY_SUB, battery->voltage_now_sub);
#endif
	/* get main/sub voltage avg */
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_AVG, SEC_DUAL_BATTERY_MAIN, battery->voltage_avg_main);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_AVG, SEC_DUAL_BATTERY_SUB, battery->voltage_avg_sub);

#if IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	/* get 3rd voltage now/avg */
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, SEC_DUAL_BATTERY_3RD, battery->voltage_now_3rd);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_VOLTAGE_AVG, SEC_DUAL_BATTERY_3RD, battery->voltage_avg_3rd);
	...
	psy_get_property(battery->pdata->sub_limiter_name,
		POWER_SUPPLY_EXT_PROP_CHG_VOLTAGE, 0, battery->chgin_sub);
	...
	psy_get_property(battery->pdata->third_limiter_name,
		POWER_SUPPLY_EXT_PROP_CHG_VOLTAGE, 0, battery->chgin_third);
...
{
#if IS_ENABLED(CONFIG_DUAL_SBP)
	/* get main/sub current now */
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_CURRENT_NOW, SEC_DUAL_BATTERY_MAIN, battery->current_now_main);
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_CURRENT_NOW, SEC_DUAL_BATTERY_SUB, battery->current_now_sub);

	/* get main/sub current avg */
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_CURRENT_AVG, SEC_DUAL_BATTERY_MAIN, battery->current_avg_main);
	psy_get_property(battery->pdata->fuelgauge_name,
		POWER_SUPPLY_EXT_PROP_DUAL_SBP_CURRENT_AVG, SEC_DUAL_BATTERY_SUB, battery->current_avg_sub);
#else
	/* get main/sub current now */
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_CURRENT_NOW, SEC_DUAL_BATTERY_MAIN, battery->current_now_main);
	psy_get_property(battery->pdata->dual_battery_name, 
		POWER_SUPPLY_PROP_CURRENT_NOW, SEC_DUAL_BATTERY_SUB, battery->current_now_sub);

	/* get main/sub current avg */
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_CURRENT_AVG, SEC_DUAL_BATTERY_MAIN, battery->current_avg_main);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_CURRENT_AVG, SEC_DUAL_BATTERY_SUB, battery->current_avg_sub);
#endif
#if IS_ENABLED(CONFIG_TRIPLE_BATTERY)
	/* get 3rd current now/avg*/
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_CURRENT_NOW, SEC_DUAL_BATTERY_3RD, battery->current_now_3rd);
	psy_get_property(battery->pdata->dual_battery_name,
		POWER_SUPPLY_PROP_CURRENT_AVG, SEC_DUAL_BATTERY_3RD, battery->current_avg_3rd);
