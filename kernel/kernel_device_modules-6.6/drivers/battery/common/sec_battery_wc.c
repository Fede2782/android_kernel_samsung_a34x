...
void sec_bat_wireless_minduty_cntl(struct sec_battery_info *battery, unsigned int duty_val)
{
	...
	// Get current tx_minduty from MFC IC
	psy_do_property(battery->pdata->wireless_charger_name, get, POWER_SUPPLY_EXT_PROP_WIRELESS_MIN_DUTY, value);
	if (value.intval >= 0)
		battery->tx_minduty = value.intval;
	...
	if (duty_val != battery->tx_minduty) {
		...
		pr_info("@Tx_Mode %s: Min duty same (%d)\n", __func__,  battery->tx_minduty);
	}
}
