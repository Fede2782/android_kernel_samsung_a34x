...
static void smcdsd_abd_save_str_array(struct abd_protect *abd,
...
void smcdsd_abd_save_str(struct abd_protect *abd, const char *print)
{
	...
	smcdsd_abd_save_str_array(abd, 1, 0, print_array);
}
...
void smcdsd_abd_save_bit(struct abd_protect *abd, unsigned int bit_size, unsigned int value, char **print)
{
...
void smcdsd_abd_mask_bit(struct abd_protect *abd, unsigned int bit_size, unsigned int value, char **print, u32 invert)
{
	...
	smcdsd_abd_save_bit(abd, bit_size, value, print_new);
}
...
static irqreturn_t smcdsd_abd_handler(int irq, void *dev_id)
{
...
#elif defined(SMCDSD_ABD_MTK_UEVENT) && defined(CONFIG_DRM_MEDIATEK_V2)
static void __smcdsd_abd_blank(struct abd_protect *abd)
{
	dbg_info("+ noti_uevent\n");

	mtk_drm_uevent_trigger(1);
...
static int __smcdsd_abd_fb_blank(struct fb_info *info, int blank)
{
	...
	smcdsd_notifier_call_chain(SMCDSD_EVENT_BLANK, &evdata);
...
static int smcdsd_abd_pin_register_handler_chain(struct abd_protect *abd,
		int id, irq_handler_t func, void *dev_id, int chain_type)
{
	...
	if (!pin) {
		dbg_info("smcdsd_abd_find_pin_info fail %d\n", id);
		return -EINVAL;
	}
		...
		irq_handler_t func, int gpio, const char *chain_name)
{
	...
	if (!pin) {
		dbg_info("smcdsd_abd_find_pin_info fail gpio: %d\n", gpio);
		return -EINVAL;
	}
	...
	while ((token = strsep(&pbuf, " ,"))) {
		...
		case CHAIN_TYPE_EVENT:
			...
			if (ret < 0)
				dbg_info("smcdsd_abd_pin_register_handler_chain fail %d\n", ret);
			break;
		}
	}
...
static void of_smcdsd_abd_pin_register_handler_chain(struct abd_protect *abd)
{
	...
	if (!np) {
		dbg_none("of_find_smcdsd_board fail\n");
		return;
	}
	...
	for_each_property_of_node(np, pp) {
		...
		for (i = 0; i < count; i++) {
			...
			gpio = of_get_gpio_with_name(np, info, NULL);
...
void smcdsd_abd_con_register(struct abd_protect *abd)
{
	struct platform_device *pdev = NULL;
	struct device_node *np = NULL;
...
static void smcdsd_abd_simple_print(struct abd_protect *abd, struct seq_file *m, void *unused)
{
	unsigned int i = 0;
		...
		unsigned long code, void *unused)
{
	...
	smcdsd_abd_simple_print(abd, NULL, NULL);
		...
		char *keyword, irq_handler_t func)
{
	...
	gpio = of_get_gpio_with_name(np, dts_name, &flags);
	if (!gpio_is_valid(gpio)) {
		dbg_info("gpio_is_valid fail, gpio: %s, %d\n", dts_name, gpio);
		goto exit;
	}
	...
	irqf_type = (flags & GPIO_ACTIVE_LOW) ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING;
	...
	if (pin->irq) {
		...
		irq_set_status_flags(pin->irq, IRQ_NOAUTOEN);
...
static int smcdsd_abd_misc_show(struct seq_file *m, void *unused)
{
	...
	smcdsd_abd_simple_print(abd, m, unused);
...
static int smcdsd_abd_misc_open(struct inode *inode, struct file *file)
{
	...
	return single_open(file, smcdsd_abd_misc_show, abd);
}

static const struct file_operations smcdsd_abd_misc_fops = {
	.read = seq_read,
	.release = single_release,
	.open = smcdsd_abd_misc_open,
};
...
static void smcdsd_abd_register_fops(struct abd_protect *abd)
{
	...
	abd->misc_entry.fops = &smcdsd_abd_misc_fops;
...
static ssize_t smcdsd_abd_debugfs_write(struct file *f, const char __user *user_buf,
...
static int smcdsd_abd_debugfs_show(struct seq_file *m, void *unused)
{
	struct abd_protect *abd = m->private;
...
static int smcdsd_abd_debugfs_open(struct inode *inode, struct file *file)
{
	return single_open(file, smcdsd_abd_debugfs_show, inode->i_private);
}

static const struct file_operations smcdsd_abd_debugfs_fops = {
	.read = seq_read,
	.release = single_release,
	.write = smcdsd_abd_debugfs_write,
	.open = smcdsd_abd_debugfs_open,
};
...
static void smcdsd_abd_register_debugfs(struct abd_protect *abd)
{
	...
	debugfs_create_file("_debug", 0644, abd_debugfs_root, abd, &smcdsd_abd_debugfs_fops);
...
#if defined(CONFIG_MEDIATEK_SOLUTION) || defined(CONFIG_ARCH_MEDIATEK)
	smcdsd_abd_register_printer(abd, mtkfb_debug_show, NULL);
#endif

	list_for_each_entry(pending, &pending_list, node) {
		smcdsd_abd_register_printer(abd, pending->printer.show, NULL);
	}
