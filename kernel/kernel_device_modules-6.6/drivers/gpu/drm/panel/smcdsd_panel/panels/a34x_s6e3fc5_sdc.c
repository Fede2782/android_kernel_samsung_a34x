	...
	struct device_attribute *attr, char *buf)
{
	...
	for_each_rx_msg_in_segment(i, MSG_S6E3FC5_SDC_CCD, ARRAY_SIZE(MSG_S6E3FC5_SDC_CCD), dsi_msg) {
		dbg_info("%2x: %*ph\n", ((u8 *)dsi_msg->tx_buf)[0], (int)dsi_msg->rx_len, (u8 *)dsi_msg->rx_buf);
...
#if defined(CONFIG_SMCDSD_MDNIE)
	lcd->mdnie_class = mdnie_register(&lcd->ld->dev, lcd, mdnie_send, mdnie_read, lcd->coordinate_xy, &tune_info);
#endif
...
static int smcdsd_panel_hbm_set_wait_state(struct platform_device *p, bool wait)
{
	...
	dev_info(&lcd->ld->dev, "%s: wait_state(%d->%d) refresh(%3u) hbm_interval(%llu) %llu\n", __func__,
