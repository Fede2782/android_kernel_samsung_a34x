...
#if defined(CONFIG_SMCDSD_MDNIE)
static int __mdnie_send(void *devdata, void *seq, u32 num)
{
	struct lcd_info *lcd = devdata;
	struct msg_segment *msg = seq;
	int ret = 0;

	struct msg_package package = { msg, num };
...
static int mdnie_send(void *devdata, void *seq, u32 num)
{
	struct lcd_info *lcd = devdata;
	struct msg_segment *msg = seq;
	int ret = 0;
...
static int mdnie_read(void *devdata, u8 addr, u8 *buf, u32 size)
{
	struct lcd_info *lcd = devdata;
	int ret = 0;
...
static int smcdsd_panel_send_msg(struct lcd_info *lcd, int force)
{
	...
	/* VRR */
	package[count++] = GET_PACKAGE(&PACKAGE_LIST[MSG_IDX_BASE][disp_mode_table[lcd->disp_mode_idx]]);
...
static int sw83109_boe_read_id(struct lcd_info *lcd)
{
	...
	if (ret < 0 || !lcd->id_value) {
		...
		if (lcdtype)
			smcdsd_abd_save_bit(&pdata->abd, BITS_PER_BYTE * LDI_LEN_ID, cpu_to_be32(lcd->id_value), LDI_BIT_DESC_ID);
	}
...
static int sw83109_boe_read_date(struct lcd_info *lcd)
{
	...
	dev_info(&lcd->ld->dev, "%s: %*ph\n", __func__, (int)ARRAY_SIZE(lcd->date), lcd->date);
...
static int sw83109_boe_read_chip_id(struct lcd_info *lcd)
{
	...
	dev_info(&lcd->ld->dev, "%s: %*ph\n", __func__, (int)ARRAY_SIZE(lcd->code), lcd->code);
...
static int sw83109_boe_read_trim(struct lcd_info *lcd)
{
	...
	if (ret < 0) {
		...
		dev_info(&lcd->ld->dev, "%s: %*ph\n", __func__, (int)ARRAY_SIZE(lcd->trim), lcd->trim);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	dev_info(&lcd->ld->dev, "%s: (%u) %*ph\n", __func__, value, (int)ARRAY_SIZE(lcd->trim), lcd->trim);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct lcd_info *lcd = dev_get_drvdata(dev);
	struct mipi_dsi_lcd_common *pdata = lcd->pdata;
	struct fb_info registered_fb = {0, };
...
static void panel_conn_register(struct lcd_info *lcd)
{
	...
	gpio = of_get_gpio_with_name(NULL, "gpio_con", NULL);
	if (gpio < 0) {
...
static int smcdsd_mdnie_force_update(struct device *dev, void *data)
{
	struct lcd_info *lcd = dev_get_drvdata(dev->parent);
	struct msg_package *package = data;
	int ret = 0;

	memset(data, 0, sizeof(struct msg_package));

	ret = mdnie_force_update(dev, data);
	if (ret < 0)
		return 0;

	__mdnie_send(lcd, package->buf, package->len);
...
#if defined(CONFIG_SMCDSD_MDNIE)
	class_for_each_device(lcd->mdnie_class, NULL, &package, smcdsd_mdnie_force_update);
#endif
...
static int smcdsd_panel_set_mode(struct platform_device *p, struct drm_display_mode *m, unsigned int dst_mode, int stage)
{
	...
	dev_info(&lcd->ld->dev, "%s: refresh(%3u) mode(%d)(%s) state(%u/%u)\n", __func__,
		drm_mode_vrefresh(m), dst_mode, m->name, lcd->state, lcd->doze_state);

//	if (m->vrefresh != 60 && m->vrefresh != 120)	/* todo */
//		return 0;

	lcd->vrefresh = drm_mode_vrefresh(m);
...
static struct lcd_device *lcd_device_create(const char *name, struct device *parent,
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	lcd->ld = lcd_device_create("panel", dev, lcd, NULL);
	if (IS_ERR(lcd->ld)) {
...
#if defined(CONFIG_SMCDSD_MDNIE)
	lcd->mdnie_class = mdnie_register(&lcd->ld->dev, lcd, mdnie_send, mdnie_read, lcd->coordinate, &tune_info);
#endif
