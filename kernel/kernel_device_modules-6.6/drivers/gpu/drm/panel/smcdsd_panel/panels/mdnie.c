...
int mdnie_force_update(struct device *dev, void *data)
{
	...
	struct mdnie_table *table = NULL;
	struct mdnie_scr_info *scr_info = mdnie->tune->scr_info;
	struct msg_package *package = (struct msg_package *)data;
	int i, ret = 0;
...
static int mdnie_check_info(struct mdnie_info *mdnie)
{
	...
	if (scr_info && night_info && night_info->max_w) {
		...
		if (limit >= table->seq[index].dsi_msg.tx_len + 1) {	/* limit is last array index of param buffer, tx_len is totoal length */
			pr_err("%s: invalid night_info offset. %d, %zu\n", __func__, limit, table->seq[index].dsi_msg.tx_len);
			ret = -EINVAL;
			goto exit;
		}
	}
	...
	if (trans_info && trans_info->enable) {
		...
		if (limit >= table->seq[index].dsi_msg.tx_len) {
			pr_err("%s: invalid trans_info offset. %d, %zu\n", __func__, limit, table->seq[index].dsi_msg.tx_len);
			ret = -EINVAL;
			goto exit;
		}
	}
...
struct class *mdnie_register(struct device *p, void *devdata,
...
int attr_store_for_each(struct class *class,
	const char *name, const char *buf, size_t size)
{
