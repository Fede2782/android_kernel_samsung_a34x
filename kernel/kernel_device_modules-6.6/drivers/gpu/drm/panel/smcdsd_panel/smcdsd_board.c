...
static struct regulator_bulk_data *find_regulator(struct action_info *find)
{
	struct dt_node_info *dt_node = NULL;
	struct list_head *lh = NULL;
	struct action_info *action = NULL;
	int idx = 0, ret = 0;
	const char *subinfo = find->subinfo;
	struct regulator_bulk_data *bulk = NULL;
	...
	while (!IS_ERR_OR_NULL(dt_nodes[idx])) {
		...
		dbg_none("%dth dt_node name is %s\n", idx, dt_node->name);
		list_for_each_entry(action, lh, node) {
			if (STRNEQ("regulator", action->type)) {
				if (action->bulk && action->bulk->supply && STREQ(action->bulk->supply, subinfo)) {
					dbg_none("%s is found in %s\n", subinfo, dt_node->name);
					return action->bulk;
				}
			}
		}
	...
	bulk = kzalloc(sizeof(struct regulator_bulk_data), GFP_KERNEL);
	bulk->supply = subinfo;

	ret = regulator_bulk_get(NULL, 1, bulk);
	if (ret < 0) {
...
static int decide_subinfo(struct device_node *np, struct action_info *action)
{
	...
	case ACTION_REGULATOR_ENABLE:
	case ACTION_REGULATOR_DISABLE:
		action->bulk = find_regulator(action);
		if (!action->bulk) {
			dbg_warn("find_regulator fail %d %s\n", ret, subinfo);
			ret = -EINVAL;
		}
		break;
	case ACTION_REGULATOR_SET_VOLTAGE:
			...
			if (action->timer->end > action->timer->now) {
				...
				if (us_delta < MSEC_TO_USEC(SMALL_MSECS)) {
					...
					if (us_delta < SHRT_MAX)
						...
						dbg_warn("ACTION_TIMER_DELAY: invalid us_delta(%lld)\n", us_delta);
				}
			}
			action->timer->end = 0;
			break;
...
int of_get_gpio_with_name(struct device_node *from, const char *gpioname, unsigned int *flags)
{
	int ret = 0, gpio = 0, count;
	struct device_node *np = NULL;
	struct of_phandle_args gpiospec;
	struct of_phandle_args out_args;

	np = from ? from : of_find_smcdsd_board(NULL);
	if (!np) {
		dbg_info("of_find_smcdsd_board fail for %s\n", gpioname);
		return -EINVAL;
	}

	gpio = of_get_named_gpio(np, gpioname, 0);
	...
	ret = of_parse_phandle_with_args(np, gpioname, "#gpio-cells", 0, &gpiospec);
	if (ret < 0) {
		dbg_info("of_parse_phandle_with_args fail %d\n", ret);
		return -EINVAL;
	}

	count = of_property_count_u32_elems(np, gpioname);
	if (count < 0 || count > MAX_PHANDLE_ARGS) {
		dbg_info("of_property_count_u32_elems count(%d)\n", count);
		return -EINVAL;
	}

	if (count <= gpiospec.args_count)	/* 3 or more <= 2 */
		return -EINVAL;

	ret = of_property_read_u32_array(np, gpioname, &out_args.args[0], count);
	if (ret < 0) {
		dbg_info("of_property_read_u32_array fail %d\n", ret);
		return -EINVAL;
	}

	*flags = out_args.args[gpiospec.args_count];
...
int of_gpio_get_value(const char *gpioname)
{
	...
	gpio = of_get_gpio_with_name(NULL, gpioname, NULL);
	if (!gpio_is_valid(gpio))
		return -EINVAL;
...
{
	int ret = 0, gpio = 0, gpio_level, active_level = 0;
	unsigned int flags = 0;

	gpio = of_get_gpio_with_name(NULL, gpioname, &flags);
	if (!gpio_is_valid(gpio)) {
		dbg_warn("of_get_gpio_with_name fail %d %s\n", gpio, gpioname);
		return -EINVAL;
	}

	active_level = !(flags & GPIO_ACTIVE_LOW);
	gpio_level = gpio_get_value(gpio);
...
static int find_panel_lut_ddi_index(void)
{
	struct device_node *parent = NULL;
	int lut_count, ret = 0;
	u32 *lut_table = NULL;
...
static int panel_lut_ddi_recommend_init(void)
{
	struct device_node *parent = NULL;
	struct device_node *np = NULL;
	int ret = 0, ddi_count;
...
int smcdsd_board_late_initcall(void)
{
