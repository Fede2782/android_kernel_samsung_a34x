...
static int common_lcd_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	for (i = 0; i < LCD_CONFIG_MAX; i++) {
		...
		mode = drm_mode_duplicate(connector->dev, &plcd->config[i].drm);
		if (!mode) {
			dbg_info("failed to add mode %ux%ux@%u\n",
				plcd->config[i].drm.hdisplay, plcd->config[i].drm.vdisplay,
				drm_mode_vrefresh(&plcd->config[i].drm));
			return -ENOMEM;
		}
	...
	connector->display_info.width_mm = plcd->config[LCD_CONFIG_DFT].lcm.physical_width;
	connector->display_info.height_mm = plcd->config[LCD_CONFIG_DFT].lcm.physical_height;
...
static int common_lcd_ext_param_set(struct drm_panel *panel,
		struct drm_connector *connector, unsigned int mode)
{
	...
	dbg_info("mode(%u) vrefresh(%dHz) match(%d)\n", mode, drm_mode_vrefresh(m), !ret);

	return ret;
}

static int common_lcd_mode_switch(struct drm_panel *panel,
...
static int common_lcd_hbm_set_lcm_cmdq(struct drm_panel *panel, void *handle, bool en)
{
	struct mipi_dsi_lcd_common *plcd = panel_to_common_lcd(panel);
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.set_disp_on_cmdq = common_lcd_set_dispon_cmdq,
#if defined(CONFIG_SMCDSD_DOZE)
...
int smcdsd_dsi_msg_tx(void *drvdata, unsigned long data0, int blocking)
{
	...
	ret = set_lcm_wrapper(NULL, dsi_msg, blocking);
	if (ret < 0) {
		dbg_info("error(%d)\n", ret);
		return -EINVAL;
	}
	...
	unsigned int id, unsigned long data0, unsigned int data1, bool need_lock)
{
	...
	ret = set_lcm_wrapper(NULL, &dsi_msg, 1);
	if (ret < 0) {
		dbg_info("error(%d)\n", ret);
		return -EINVAL;
	}
...
//	lcd_driver_init();
	...
	for (i = 0; i < LCD_CONFIG_MAX; i++)
		plcd->config[i].ext.drm_panel = &plcd->panel;

	drm_panel_init(&plcd->panel, dev, &common_lcd_drm_funcs, DRM_MODE_CONNECTOR_DSI);

	drm_panel_add(&plcd->panel);
...
static void common_lcd_remove(struct mipi_dsi_device *dsi)
{
