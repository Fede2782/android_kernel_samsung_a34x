...
static void __mt_gpufreq_setup_opp_power_table(int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		GPUFREQ_LOGE("[%d], freq_khz = %u, volt = %u, power = %u\n",
...
int __gpufreq_power_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON && g_gpu.power_count == 1) {
		...
		ret = __gpufreq_cg_control(GPU_PWR_ON);
		if (unlikely(ret)) {
		...
		ret = __gpufreq_cg_control(GPU_PWR_OFF);
		if (unlikely(ret)) {
...
int __gpufreq_fix_target_oppidx_gpu(int oppidx)
{
	...
	if (oppidx == -1) {
		...
		__gpufreq_generic_commit_gpu(g_gpu.min_oppidx, DVFS_FIX_OPP);
	...
	unsigned int freq, unsigned int volt)
{
	...
	if (freq == 0 && volt == 0) {
		...
		/* we don't care the result of this commit */
		__gpufreq_generic_commit_gpu(g_gpu.min_oppidx, DVFS_FIX_FREQ_VOLT);
...
void __gpufreq_resume_dvfs(void)
{
	...
	ret = __gpufreq_power_control(GPU_PWR_OFF);
	if (unlikely(ret < 0)) {
...
int __gpufreq_pause_dvfs(void)
{
	...
	if (unlikely(ret < 0)) {
		GPUFREQ_LOGE("fail to control power state: %d (%d)", GPU_PWR_ON, ret);
		mutex_lock(&gpufreq_lock_gpu);
		g_dvfs_state &= ~DVFS_AGING_KEEP;
		mutex_unlock(&gpufreq_lock_gpu);
		goto done;
	}

	ret = __gpufreq_custom_commit_gpu(keep_freq, keep_volt, DVFS_DISABLE);
	if (unlikely(ret)) {
		GPUFREQ_LOGE("fail to custom commit GPU freq: %d, volt: %d (%d)",
...
static int __gpufreq_switch_clksrc(enum gpufreq_clk_src clksrc)
{
	...
	if (clksrc == CLOCK_MAIN) {
		...
		if (unlikely(ret)) {
			__gpufreq_abort("fail to switch to CLOCK_MAIN (%d)", ret);
			goto done;
		}
	} else if (clksrc == CLOCK_SUB) {
		ret = clk_set_parent(g_clk->clk_mux, g_clk->clk_sub_parent);
		if (unlikely(ret)) {
			__gpufreq_abort("fail to switch to CLOCK_SUB (%d)", ret);
			goto done;
		}
	} else {
	...
	unsigned int vsram_old, unsigned int vsram_new)
{
	...
	if (vgpu_new > vgpu_old) {
		...
		if (unlikely(ret)) {
			__gpufreq_abort("fail to set VSRAM (%d)", ret);
			goto done;
		}
		...
		if (g_sleep) {
			if (g_shader_present == GPU_SHADER_PRESENT_9) {
				...
				if (unlikely(val != 0x1FC)) {
					__gpufreq_abort("incorrect mtcmos power on status: 0x%08x", val);
					ret = GPUFREQ_EINVAL;
					goto done;
				}
			}
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	for (i = 0; i < g_gpu.opp_num; i++) {
		...
		g_gpu.working_table[i].margin = g_gpu.signed_table[j].margin;
		g_gpu.working_table[i].power = g_gpu.signed_table[j].power;
		GPUFREQ_LOGD("[%02d] freq: %d, volt: %d, vsram: %d, margin: %d",
...
static int __gpufreq_init_clk(struct platform_device *pdev)
{
	...
	if (IS_ERR(g_clk->clk_mux)) {
		__gpufreq_abort("fail to get clk_mux (%ld)", PTR_ERR(g_clk->clk_mux));
	...
	if (IS_ERR(g_clk->clk_main_parent)) {
		__gpufreq_abort("fail to get clk_main_parent (%ld)", PTR_ERR(g_clk->clk_main_parent));
	...
	if (IS_ERR(g_clk->clk_sub_parent)) {
		__gpufreq_abort("fail to get clk_sub_parent (%ld)", PTR_ERR(g_clk->clk_sub_parent));
...
static int __gpufreq_init_pmic(struct platform_device *pdev)
{
	...
	if (IS_ERR(g_pmic->reg_vgpu)) {
		...
		__gpufreq_abort("fail to get VGPU (%ld)",
	...
	if (IS_ERR(g_pmic->reg_vsram)) {
		...
		__gpufreq_abort("fail to get VSRAM (%ld)",
