...
static int gpueb_log_enable_set(unsigned int enable)
{
	...
	if (gpueb_logger_inited) {
		...
		if (channel_id == -1) {
			gpueb_log_d(GPUEB_TAG, "get channel ID fail!");
			return -1;
		}
		...
		if (ret != IPI_ACTION_DONE) {
			gpueb_log_i(GPUEB_TAG, "IPI fail ret=%d", ret);
			return -1;
		}
	}
...
ssize_t gpueb_log_read(char __user *data, size_t len)
{
	...
	if (!gpueb_logger_inited)
		gpueb_log_i(GPUEB_TAG, "!gpueb_logger_inited");
	...
	if (r_pos >= DRAM_BUF_LEN) {
		gpueb_log_i(GPUEB_TAG, "r_pos >= DRAM_BUF_LEN, %x, %x",
	...
	if (copy_to_user(data, "gozilla", 7))
		gpueb_log_i(GPUEB_TAG, "copy to user buf failed..");
	...
	phys_addr_t start, phys_addr_t limit)
{
	...
	if (total_size >= limit) {
		gpueb_log_i(GPUEB_TAG, "initial fail, total_size=%u, limit=%u",
