...
static bool pd_is_src_usb_suspend_support(struct adapter_device *dev)
{
	...
	return tcpm_is_src_usb_suspend_support(info->tcpc[active_idx]);
}
...
static bool pd_is_src_usb_communication_capable(struct adapter_device *dev)
{
	...
	return tcpm_inquire_usb_comm(info->tcpc[active_idx]);
}
#endif
				...
				int active_idx, struct adapter_power_cap *cap)
{
	...
	for (i = 0, j = 0; i < pd_cap.nr && j < ADAPTER_CAP_MAX_NR; i++) {
		...
		if (pd_cap.type[i] == 0)
			...
			cap->type[i] = MTK_PD_VPDO;
		else if (pd_cap.type[i] == 3)
			cap->type[j] = MTK_PD_APDO;
		else
			cap->type[j] = MTK_CAP_TYPE_UNKNOWN;
#else
		cap->type[j] = MTK_PD;
#endif
		j++;
