...
static void mt6360_wake_up_charger(struct mt6360_chg_info *mci)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		dev_err(mci->dev, "%s Couldn't get chg_psy\n", __func__);
	} else {
		info = (struct mtk_charger *)power_supply_get_drvdata(chg_psy);
	if (info == NULL) {
		dev_err(mci->dev, "%s Couldn't get mtk_charger\n", __func__);
		return;
	}
...
#endif
		cancel_delayed_work(&mci->sec_cable_work);
...
static int mt6360_chgdet_post_process(struct mt6360_chg_info *mci)
{
	...
	mci->attach = attach;
	dev_info(mci->dev, "%s: vt_muic : attach = %d\n", __func__, attach);
	...
	if (usb_status == MT6360_CHG_TYPE_SDPNSTD) {
		if (atomic_read(&mci->bc12_nsdp_cnt) < 1) {
			ret = mt6360_toggle_usbchgen(mci);
			/* Workaround success, wait for next event */
			if (ret >= 0) {
				atomic_inc(&mci->bc12_nsdp_cnt);
				return ret;
			}
		}
		atomic_set(&mci->bc12_nsdp_cnt, 0);
...
static int __mt6360_set_aicr(struct mt6360_chg_info *mci, u32 uA)
{
	...
	if (mci->f_mode == OB_MODE) {
		dev_err(mci->dev, "%s: do not set aicr for no bat boot support\n", __func__);
		return ret;
	}
#endif
...
static void mt6360_sec_cable_work_func(struct work_struct *work)
{
	...
	pr_info("%s: vt_muic with: sec_cable_type(%d), psy_desc.type(%d), psy_usb_type(%d)\n",
		__func__, mci->sec_cable_type, mci->psy_desc.type, mci->psy_usb_type);
	value.intval = mci->sec_cable_type;
...
#if defined(CONFIG_MTK_NO_BAT_BOOT_SUPPORT)
	if (mci->f_mode == OB_MODE) {
		dev_err(mci->dev, "%s: no battery. do not set power path\n", __func__);
		return 0;
	}
#endif
	dev_dbg(mci->dev, "%s: en = %d\n", __func__, en);
...
static int mt6360_is_charging_eoc(struct charger_device *chg_dev, bool *is_eoc)
{
	struct mt6360_chg_info *mci = charger_get_data(chg_dev);
	...
	ret = regmap_read(mci->regmap, MT6360_PMU_CHG_STAT5, &regval);
	if (ret < 0)
		return ret;
	ieoc_stat = (regval & MT6360_CHG_IEOCI_MASK);

	if (ieoc_stat)
		dev_info(mci->dev, "%s: FULL\n", __func__);
...
static int mt6360_get_chg_status(struct charger_device *chg_dev, u32 *charging_status)
{
	struct mt6360_chg_info *mci = charger_get_data(chg_dev);
	int ret = 0, pwr_rdy;
	bool chg_en = false;
...
static const struct charger_ops mt6360_chg_ops = {
	...
	.get_charging_status = mt6360_get_chg_status,
#endif
};
...
#if defined(CONFIG_MTK_NO_BAT_BOOT_SUPPORT)
	if (mci->f_mode == OB_MODE) {
		dev_err(mci->dev, "%s: force return for no battery boot support.\n", __func__);
		return 0;
	}
#endif
...
static int mt6360_chg_init_setting(struct mt6360_chg_info *mci)
{
	...
	if (!f_mode)
		...
		mci->f_mode = NO_MODE;
	pr_info("[BAT] %s: f_mode: %s\n", __func__, BOOT_MODE_STRING[mci->f_mode]);
#endif
	...
	ret = regmap_update_bits(mci->regmap,
					...
					0x03);
	if (ret < 0) {
		dev_dbg(mci->dev, "%s: MT6360_MASK_VRECH set fail\n", __func__);
		return ret;
	}

	/* disable charging before bc12 detection.. #Charging, */
	/* ALPS08349619 : lpm => power on => cannot recognize bc12 cable.*/
	ret = regmap_update_bits(mci->regmap,
				 MT6360_PMU_CHG_CTRL2,
				 MT6360_CHG_EN_MASK, 0);
	if (ret < 0)
		dev_notice(mci->dev, "%s: failed to set MT6360_CHG_EN as disable\n", __func__);
#endif
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		ret = mt6360_get_vsys(mci->chg_dev, &val->intval);
		break;
#endif
...
static int mt6360_pmu_chg_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: entry. mt6360 gauge probe ended.\n", __func__);
