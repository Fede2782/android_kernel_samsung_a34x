...
int pdc_clear(void)
{
	...
	if (pd_noti.sink_status.has_apdo) {
		...
		tcpm_reset_pd_charging_policy(tcpc, NULL);
...
int pdc_send_pd_noti(void)
{
	...
	for (i = 0; i < temp; i++) {
		...
		pr_info("%s : PDO_Num[%d,%s,%s] MAX_CURR(%d) MAX_VOLT(%d), AVAILABLE_PDO_Num(%d), comm(%d), suspend(%d)\n",
			__func__, i, pd_noti.sink_status.power_list[i + 1].apdo ? "APDO" :
	...
	pr_info("%s : [END] pd_noti.event = %d, current_pdo_num(%d)\n", __func__,
		pd_noti.event, pd_noti.sink_status.current_pdo_num);
	pdic_noti.sub1 = 1;
	pdic_noti.sub2 = 0;
	pdic_noti.sub3 = 0;
	pdic_noti.pd = NULL;
			...
			int *newidx)
{
	...
	*newcur = cap->ma[*newidx];
	...
	if (tcpm_inquire_typec_attach_state(pd->tcpc_dev) == TYPEC_ATTACHED_SNK)
...
static int __pd_run(struct chg_alg_device *alg)
{
	struct mtk_pd *pd = dev_get_drvdata(&alg->dev);
	int vbus = 0;
	int cur = 0, idx = 0, ret = 0, ret_value = ALG_RUNNING;
