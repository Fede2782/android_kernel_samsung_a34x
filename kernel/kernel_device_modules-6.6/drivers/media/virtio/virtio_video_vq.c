...
#if IS_ENABLED(CONFIG_VIRTIO_VIDEO_MTK_EXTENSION)
		virtio_video_queue_error_event(stream);
#endif
		break;

#if IS_ENABLED(CONFIG_VIRTIO_VIDEO_MTK_EXTENSION)
	case VIRTIO_VIDEO_EVENT_EOS:
		virtio_v4l2_err("eos on stream %d\n", stream_id);
				   ...
				   const char *tag)
{
	...
	if (strscpy(req_p->tag, tag, sizeof(req_p->tag) - 1) < 0)
		virtio_v4l2_err("failed to copy stream tag\n");
	...
	struct virtio_video_mem_entry *ents)
{
	...
	req_p = virtio_video_alloc_req(vv, &vbuf, sizeof(*req_p));
	if (IS_ERR_OR_NULL(req_p)) {
		virtio_v4l2_err("[%d]req_p fail, ret %ld\n", stream_id, PTR_ERR(req_p));
		return PTR_ERR(req_p);
	}

	req_p->hdr.type = cpu_to_le32(VIRTIO_VIDEO_CMD_RESOURCE_CREATE_MTK);
	req_p->hdr.stream_id = cpu_to_le32(stream_id);
	req_p->num_planes = cpu_to_le32(1);
	req_p->num_entries[0] = cpu_to_le32(num_entries);

	vbuf->data_buf = ents;
	vbuf->data_size = sizeof(*ents) * num_entries;

	virtio_v4l2_debug(VIRTIO_DBG_L1, "[%d]resource_create_page_mtk\n", stream_id);
				    ...
				    uint8_t num_data_size, uint32_t queue_type)
{
	...
	req_p->flags = virtio_vb->v4l2_m2m_vb.vb.flags;
...
#if IS_ENABLED(CONFIG_VIRTIO_VIDEO_MTK_EXTENSION)
static void virtio_video_cmd_get_ctrl_mtk_cb(struct virtio_video *vv,
					     struct virtio_video_vbuffer *vbuf)
{
	struct virtio_video_stream *stream = vbuf->priv;
	struct video_control_info *control = &stream->control;
...
static int virtio_video_cmd_get_control_mtk(struct virtio_video *vv,
					    struct virtio_video_stream *stream,
					    ...
					    void *out_buf, size_t out_buf_size)
{
	...
	struct virtio_video_get_control_resp *resp_p = NULL;
	struct virtio_video_vbuffer *vbuf = NULL;
	size_t req_size = sizeof(struct virtio_video_get_control) +
			  sizeof(struct virtio_video_control_val_mtk);
	size_t resp_size =
		sizeof(struct virtio_video_get_control_resp) + out_buf_size;
	virtio_video_resp_cb cb = &virtio_video_cmd_get_ctrl_mtk_cb;
	struct virtio_video_control_val_mtk *req_mtk = NULL;
	uint8_t *resp_data = NULL;

	if (!vv) {
		virtio_v4l2_err("[%d]vv is null\n", stream->stream_id);
		return -1;
	}

	req_p = virtio_video_alloc_req_resp(vv, cb, &vbuf, req_size, resp_size,
					    NULL);
	if (IS_ERR_OR_NULL(req_p)) {
		virtio_v4l2_err("[%d]req_p fail, ret %ld\n", stream->stream_id, PTR_ERR(req_p));
		return PTR_ERR(req_p);
	}

	req_p->hdr.type = cpu_to_le32(VIRTIO_VIDEO_CMD_GET_CONTROL);
	req_p->hdr.stream_id = cpu_to_le32(stream->stream_id);
	req_p->control = cpu_to_le32(VIRTIO_VIDEO_CONTROL_MTK_EXTENSION);

	req_mtk = (void *)((char *)req_p +
			   sizeof(struct virtio_video_get_control));
	req_mtk->id = cpu_to_le32(id);
	req_mtk->type = cpu_to_le32(type);
	req_mtk->payload_size = cpu_to_le32(out_buf_size);
	...
	if (ret) {
		free_vbuf(vv, vbuf);
		virtio_v4l2_err("[%d]queue_cmd_buffer err:%d\n", stream->stream_id, ret);
		return ret;
	}
	...
	if (ret == 0) {
		free_vbuf(vv, vbuf);
		virtio_v4l2_err("timed out waiting for get_params\n");
		return -1;
	}

	resp_data = (void *)((char *)resp_p +
			     sizeof(struct virtio_video_get_control_resp));
	memcpy(out_buf, resp_data, out_buf_size);
	return 0;
}
...
{
	return virtio_video_cmd_get_control_mtk(
		vv, stream, id, V4L2_CTRL_TYPE_INTEGER, value, sizeof(*value));
}
...
{
	return virtio_video_cmd_get_control_mtk(
		vv, stream, id, V4L2_CTRL_TYPE_U32, value, value_len);
}
...
{
	return virtio_video_cmd_get_control_mtk(
		vv, stream, id, V4L2_CTRL_TYPE_STRING, value, strlen(value) + 1);
}

static int virtio_video_cmd_set_control_mtk(struct virtio_video *vv,
					    uint32_t stream_id, uint32_t id,
					    uint32_t type, void *payload,
					    size_t payload_len)
{
	struct virtio_video_set_control *req_p = NULL;
	struct virtio_video_vbuffer *vbuf = NULL;
	struct virtio_video_control_val_mtk *mtk_p = NULL;
	uint8_t *payload_p = NULL;
	size_t size = sizeof(struct virtio_video_control_val_mtk) + payload_len;

	if (!vv || payload == 0) {
		virtio_v4l2_err("[%d]vv or payload is null\n", stream_id);
		return -EINVAL;
	}
	...
	mtk_p = (void *)((char *)req_p +
			 sizeof(struct virtio_video_set_control));
	mtk_p->id = cpu_to_le32(id);
	mtk_p->type = cpu_to_le32(type);
	mtk_p->payload_size = cpu_to_le32(payload_len);

	payload_p = (void *)((char *)mtk_p +
			     sizeof(struct virtio_video_control_val_mtk));
	memcpy(payload_p, payload, payload_len);
	return virtio_video_queue_cmd_buffer(vv, vbuf);
}
...
int virtio_video_cmd_set_control_mtk_integer(struct virtio_video *vv,
					     ...
					     int32_t value)
{
	return virtio_video_cmd_set_control_mtk(vv, stream_id, id,
