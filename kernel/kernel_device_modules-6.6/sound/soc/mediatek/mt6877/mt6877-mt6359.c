...
static int i2s3_init(struct snd_soc_pcm_runtime *rtd)
{
...
#endif
#if IS_ENABLED(CONFIG_DEVICE_MODULES_SND_SOC_CS35L41)
		if (strstr(component->name, "cs35l41"))
					       ...
					       struct snd_pcm_hw_params *params)
{
	...
	ret = mt6877_mt6359_i2s_hw_params(substream, params);
	if (ret < 0) {
		dev_err(card->dev, "set cpu dai sysclk failed: %d\n", ret);
		return ret;
	}
