		      ...
		      unsigned int alg, void *buf, size_t len)
{
	...
	/* return if lock cannot be obtained immediately */
	if (down_read_trylock(&dsp->component->card->snd_card->controls_rwsem)) {
		up_read(&dsp->component->card->snd_card->controls_rwsem);
	} else {
		return 0;
	}
