...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_SND_SOC_CIRRUS_AMP)
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_info(cs35l45->dev, "%s event : %x\n", __func__, event);
...
static const struct snd_kcontrol_new ngate2_en_ctl =
	SOC_DAPM_SINGLE("Switch", SND_SOC_NOPM, 0, 1, 0);
...
static const struct snd_soc_dapm_widget cs35l45_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SWITCH("NGATE2 Enable", CS35L45_MIXER_NGATE_CH2_CFG, 16, 0, &ngate2_en_ctl),
...
static const struct snd_soc_dapm_route cs35l45_dapm_routes[] = {
	...
	{"NGATE1 Enable", "Switch", "NGATE_CH1"},
...
static void cs35l45_log_status(struct cs35l45_private *cs35l45, int mute)
{
	static unsigned int regs[] = {
		CS35L45_BLOCK_ENABLES,	CS35L45_BLOCK_ENABLES2,
		CS35L45_IRQ1_STS_1, CS35L45_IRQ1_STS_2,
		CS35L45_IRQ1_STS_3, CS35L45_IRQ1_STS_4,
		CS35L45_IRQ1_STS_5, CS35L45_IRQ1_STS_7,
		CS35L45_IRQ1_STS_8, CS35L45_IRQ1_STS_18,
	};
...
static int cs35l45_pcm_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	struct cs35l45_private *cs35l45 =
				snd_soc_component_get_drvdata(dai->component);

	dev_info(cs35l45->dev, "%s mute=%d\n", __func__, mute);
...
static int cs35l45_component_probe(struct snd_soc_component *component)
{
	...
	static struct reg_sequence cs35l45_cal_pre_config[] = {
		{CS35L45_BOOST_OV_CFG,		0x003000D0},
		{CS35L45_BOOST_UV_CFG,		0x00000470},
		{CS35L45_BOOST_DCM_CFG,		0x00710220},
		{CS35L45_BOOST_LPMODE_CFG,	0x00000001},
		{CS35L45_HVLV_CONFIG,		0x00520017},
		{CS35L45_DSP1RX6_INPUT,		CS35L45_PCM_SRC_VDD_BSTMON},
		{CS35L45_MIXER_NGATE_CH1_CFG,	0},
		{CS35L45_MIXER_NGATE_CH2_CFG,	0},
		{CS35L45_LDPM_CONFIG,		0},
	};
	...
	cs35l45_cal_pre_config[6].def |= val << CS35L45_AUX_NGATE_CH_HOLD_SHIFT;
	...
	cs35l45_cal_pre_config[6].def |= val << CS35L45_AUX_NGATE_CH_THR_SHIFT;
	...
	cs35l45_cal_pre_config[7].def |= val << CS35L45_AUX_NGATE_CH_HOLD_SHIFT;
	...
	cs35l45_cal_pre_config[7].def |= val << CS35L45_AUX_NGATE_CH_THR_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp1_amp_sel & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp1_amp_sel & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP1_AMP_SEL_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP1_AMP_SEL_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp1_boost_sel & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp1_boost_sel & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP1_BOOST_SEL_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP1_BOOST_SEL_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp1_delay & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp1_delay & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP1_DELAY_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP1_DELAY_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp1_pcm_thld & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp1_pcm_thld & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP1_PCM_THLD_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP1_PCM_THLD_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp2_imon_sel & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp2_imon_sel & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP2_IMON_SEL_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP2_IMON_SEL_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp2_vmon_sel & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp2_vmon_sel & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP2_VMON_SEL_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP2_VMON_SEL_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp2_delay & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp2_delay & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP2_DELAY_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP2_DELAY_SHIFT;

	if (pdata->ldpm_cfg.ldpm_gp2_pcm_thld & CS35L45_VALID_PDATA)
		val = pdata->ldpm_cfg.ldpm_gp2_pcm_thld & (~CS35L45_VALID_PDATA);
	else
		val = CS35L45_LDPM_GP2_PCM_THLD_DEFAULT;

	cs35l45_cal_post_config[6].def |= val << CS35L45_LDPM_GP2_PCM_THLD_SHIFT;
	...
	amp_cfg.runtime_pm = cs35l45->pdata.allow_hibernate;
...
static irqreturn_t cs35l45_pll_unlock(int irq, void *data)
{
	...
	dev_info(cs35l45->dev, "PLL unlock flag rise detected!");
...
static irqreturn_t cs35l45_pll_lock(int irq, void *data)
{
	...
	dev_info(cs35l45->dev, "PLL lock detected!");
...
int cs35l45_suspend_runtime(struct device *dev)
{
	...
	if (cs35l45->dsp.cs_dsp.booted && cs35l45->pdata.allow_hibernate)
		ret = cs35l45_hibernate(cs35l45, HIBER_MODE_EN);
	mutex_unlock(&cs35l45->hb_lock);
...
int cs35l45_resume_runtime(struct device *dev)
{
	...
	mutex_lock(&cs35l45->hb_lock);
	if (cs35l45->dsp.cs_dsp.booted && cs35l45->pdata.allow_hibernate)
		ret = cs35l45_hibernate(cs35l45, HIBER_MODE_DIS);
	mutex_unlock(&cs35l45->hb_lock);
...
static int cs35l45_hibernate(struct cs35l45_private *cs35l45, bool hiber_en)
{
	...
	if (hiber_en == HIBER_MODE_EN) {
		...
		regmap_write(cs35l45->regmap, CS35L45_IRQ1_MASK_2, 0xffffffff);

		cmd = CSPL_MBOX_CMD_HIBERNATE;
		regmap_write(cs35l45->regmap, CS35L45_DSP_VIRT1_MBOX_1, cmd);

		down_read(&cs35l45->component->card->snd_card->controls_rwsem);
		ret = cs35l45_activate_ctl(cs35l45, "DSP1 Preload Switch",
					   false);
		if (ret < 0)
			dev_err(cs35l45->dev, "Unable to deactivate ctl (%d)\n",
				ret);
		up_read(&cs35l45->component->card->snd_card->controls_rwsem);
