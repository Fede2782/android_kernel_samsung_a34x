 ...
 * #define pr_debug(...) {\
 *  FILE *stream;\
 *  if ((stream = freopen("tfa_debug.txt", "ab+", stdout)) == NULL)\
 *    exit(-1);\
 *    printf(__VA_ARGS__);\
 ...
 * #define PRINT_ASSERT(e) {\
 *  if (e)\
 *   fprintf(stderr, "PrintAssert:%s (%s:%d) %s\n",\
...
#include "app_global.h"
	...
	#define _DEBUG(level, fmt, va...) \
		TB_TRACE_INF(TbTracePfx2("tfa", TB_FUNC, va))
#else
	#define _DEBUG(level, fmt, va...) do {} while (0)
#endif
...
#define _ERRORMSG(fmt, va...) \
	fprintf(stderr, PREFIX "ERROR %s:%s:%d: "fmt,\
