...
#define psy_get_property(name, property, propval, value) \
({	\
	struct power_supply *psy;	\
	int ret = 0;	\
	union power_supply_propval psyval = {0, };	\
	psy = get_power_supply_by_name((name));	\
	if (!psy) {	\
		...
		if (psy->desc->get_property != NULL) { \
			psyval.intval = propval;	\
			ret = psy->desc->get_property(psy, \
				(enum power_supply_property) (property), &(psyval)); \
			if (ret < 0) {	\
				pr_err("%s: Fail to %s get "#property" (%d)\n", \
						__func__, name, ret);	\
				value = 0;	\
			} else {	\
				value = psyval.intval;	\
...
#define psy_set_property(name, property, value) \
({	\
	struct power_supply *psy;	\
	int ret = 0;	\
	union power_supply_propval psyval = {0, };	\
	...
	if (!psy) {	\
		...
		if (psy->desc->set_property != NULL) { \
			...
			ret = psy->desc->set_property(psy, \
				(enum power_supply_property) (property), &(psyval)); \
			if (ret < 0) {	\
				pr_err("%s: Fail to %s set "#property" (%d)\n", \
						__func__, name, ret);	\
			}	\
		} else {	\
			ret = -ENOSYS;	\
