 ...
 *============================================================================
 ****************************************************************************/
#include "ov13a10fumipiraw_Sensor.h"
#include "adaptor-subdrv.h"
static u16 get_gain2reg(u32 gain);
static int init_ctx(struct subdrv_ctx *ctx, struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt);
static int ov13a10fu_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov13a10fu_set_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void ov13a10fu_set_streaming_control(struct subdrv_ctx *ctx, bool enable);
static int ov13a10fu_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov13a10fu_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int ov13a10fu_open(struct subdrv_ctx *ctx);
static int ov13a10fu_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static u32 ov13a10fu_dgain2reg(struct subdrv_ctx *ctx, u32 dgain);
static int ov13a10fu_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov13a10fu_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov13a10fu_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_CHECK_SENSOR_ID, ov13a10fu_check_sensor_id},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, ov13a10fu_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, ov13a10fu_set_streaming_suspend},
	{SENSOR_FEATURE_SET_MULTI_DIG_GAIN, ov13a10fu_set_multi_dig_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, ov13a10fu_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, ov13a10fu_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, ov13a10fu_set_multi_shutter_frame_length},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	/* frame_desc_prev */
	{
		...
		.mode_setting_table = ov13a10fu_preview_setting,
		.mode_setting_len = ARRAY_SIZE(ov13a10fu_preview_setting),
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 1,
	.ana_gain_step = 64,
	.ana_gain_table = ov13a10fu_ana_gain_table,
	.ana_gain_table_size = sizeof(ov13a10fu_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	.exposure_max = 0xFFFF,
	...
	.init_setting_table = ov13a10fu_init_setting,
	.init_setting_len = ARRAY_SIZE(ov13a10fu_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = ov13a10fu_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = ov13a10fu_open,
...
const struct subdrv_entry ov13a10fu_mipi_raw_entry = {
	.name = "ov13a10fu_mipi_raw",
	.id = OV13A10FU_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int ov13a10fu_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = ov13a10fu_dgain2reg(ctx, gains[i]);
	}
...
static int ov13a10fu_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (*feature_data) {
		if (ctx->s_ctx.aov_sensor_support &&
			...
			*(feature_data + 1) = 0;
			ov13a10fu_set_shutter_frame_length(ctx, para, len);
		}
	}

	ov13a10fu_set_streaming_control(ctx, TRUE);
	return 0;
}
static int ov13a10fu_set_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	ov13a10fu_set_streaming_control(ctx, FALSE);
	return 0;
}

static int ov13a10fu_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return ov13a10fu_get_imgsensor_id(ctx, (u32 *)para);
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == OV13A10_SENSOR_ID) {
...
static int ov13a10fu_open(struct subdrv_ctx *ctx)
{
	...
	/* get sensor id */
	if (ov13a10fu_get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
...
static int ov13a10fu_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.aov_sensor_support &&
		...
		*(feature_data + 1) = 0;
		return ov13a10fu_set_shutter_frame_length(ctx, para, len);
	}
