...
static int s5k3p9sp_open(struct subdrv_ctx *ctx);
static int s5k3p9sp_set_ctrl_locker(struct subdrv_ctx *ctx, u32 cid, bool *is_lock);
static int s5k3p9sp_control(struct subdrv_ctx *ctx,
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (ctx->s_ctx.aov_sensor_support && ctx->s_ctx.mode[scenario_id].aov_mode) {
