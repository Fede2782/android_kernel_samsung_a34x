...
static int hi847_set_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	...
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, hi847_set_streaming_suspend},
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	// frame_desc_cus1
	{
		...
		.delay_frame = 2,
		.csi_param = {
			.dphy_trail = 90, // Fill correct Ths-trail (in ns unit) value
		},
		.dpc_enabled = true,
	},
...
static int hi847_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	/* 1:Solid Color 2:Color Bar 5: Black pattern */
	if (mode == 0x1)
		subdrv_i2c_wr_u16(ctx, 0x0C0A, (mode << 8) & 0xff00);
	else if (mode == 0x5) {
		/* black setting */
		subdrv_i2c_wr_u16(ctx, 0x0C0C, 0x0); /* R */
		subdrv_i2c_wr_u16(ctx, 0x0C0E, 0x0); /* Gr */
		subdrv_i2c_wr_u16(ctx, 0x0C10, 0x0); /* B */
		subdrv_i2c_wr_u16(ctx, 0x0C12, 0x0); /* Gb */

		/* solid pattern setting */
		subdrv_i2c_wr_u16(ctx, 0x0C0A, (0x1 << 8) & 0xff00);
		DRV_LOG_MUST(ctx, "[ksh] black pattern enabled\n");
	} else
