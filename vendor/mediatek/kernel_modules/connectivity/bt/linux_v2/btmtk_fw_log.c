...
ssize_t btmtk_fops_writefwlog(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (skb->len > 2 && skb->data[0] == 0x01) {
		if (skb->data[1] == 0x6F && skb->data[2] == 0xFC) {
			...
			bmain_info->dbg_send_opcode[1] = 0x00;
			BTMTK_INFO("DBG_SEND_VENDOR_CMD opcode is [%02x]", bmain_info->dbg_send_opcode[0]);
			bmain_info->dbg_send = DBG_SEND_VENDOR_CMD;
		} else {
			memcpy(bmain_info->dbg_send_opcode, (skb->data + 1), 2);
			BTMTK_INFO("DBG_SEND_HCI_CMD opcode is [%02x, %02x]", bmain_info->dbg_send_opcode[0],
					bmain_info->dbg_send_opcode[1]);
			bmain_info->dbg_send = DBG_SEND_HCI_CMD;
		}
	}
...
static void btmtk_fwdump_wake_lock(void)
{
	...
	__pm_wakeup_event(bmain_info->fwdump_ws, 15500);
...
coredump_fail_unlock:
		...
		if (bmain_info->hif_hook.log_handler) {
			bmain_info->hif_hook.log_handler(skb->data + 4, skb->len - 4);
			/* send to connv3 only instead sending into stack */
			return 1;
		}
		...
		return 1;
	} else if (bmain_info->dbg_send == DBG_SEND_HCI_CMD && bt_cb(skb)->pkt_type == HCI_EVENT_PKT &&
				skb->len > 4 &&
				skb->data[3] == bmain_info->dbg_send_opcode[0] &&
				skb->data[4] == bmain_info->dbg_send_opcode[1]) {
		bmain_info->dbg_send = DBG_SEND_NONE;
		BTMTK_INFO_RAW(skb->data, skb->len, "%s: dbg_node_event len[%d] %02x", __func__,
					skb->len + 1, hci_skb_pkt_type(skb));
		return 1;
	} else if (bmain_info->dbg_send == DBG_SEND_HCI_CMD && bt_cb(skb)->pkt_type == HCI_EVENT_PKT &&
				skb->len > 5 && skb->data[0] == 0x0F &&
				skb->data[4] == bmain_info->dbg_send_opcode[0] &&
				skb->data[5] == bmain_info->dbg_send_opcode[1]) {
		bmain_info->dbg_send = DBG_SEND_NONE;
		BTMTK_INFO_RAW(skb->data, skb->len, "%s: status dbg_node_event len[%d] %02x", __func__,
					skb->len + 1, hci_skb_pkt_type(skb));
		return 1;
	} else if (bmain_info->dbg_send == DBG_SEND_VENDOR_CMD && bt_cb(skb)->pkt_type == HCI_EVENT_PKT &&
				skb->len > 3 && skb->data[0] == 0xE4 &&
		...
		return 1;
	} else if (bt_cb(skb)->pkt_type == HCI_EVENT_PKT && skb->len == 6
				&& skb->data[0] == 0x04 && skb->data[1] == 0x04
				&& skb->data[2] == 0x04 && skb->data[3] == 0x04
				&& skb->data[4] == 0x04 && skb->data[5] == 0x04) {
		BTMTK_INFO_RAW(skb->data, skb->len, "%s: Invalid 0x04, ignore:", __func__);
		return 1;
	}
