...
int consys_conninfra_on_power_ctrl_mt6877(unsigned int enable)
{
	...
	unsigned int conninfra_off_iso = 0, value_180602c8 = 0;
	int check;
	...
	/* 0x1806_015c[2:0] = 3b'000, read and check 0x1806_02c8[27]==1b'1 */
	CONSYS_REG_WRITE(CONN_HOST_CSR_TOP_CONN_INFRA_CFG_DBG_SEL_ADDR, 0x4);
	value_180602c8 = CONSYS_REG_READ(CONN_HOST_CSR_TOP_DBG_DUMMY_2_ADDR);
	pr_info("%s[%d], check conninfra off iso before turn off MTCMOS: [0x%08x][0x%08x]\n",
		__func__, __LINE__, conninfra_off_iso, value_180602c8);
	if (((conninfra_off_iso & 0x10000) == 0x10000) && ((conninfra_off_iso & 0x0700) == 0) &&
	    ((value_180602c8 & 0x08000000) == 0x08000000)) {
         	pr_info("[%s] special off flow\n", __func__);
