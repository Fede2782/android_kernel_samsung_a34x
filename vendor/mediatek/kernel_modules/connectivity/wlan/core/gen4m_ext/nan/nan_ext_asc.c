				...
				struct NAN_ASC_ASYNC_EVENT *prAscAsyncEvt)
{
	...
	if (status == NAN_ASC_STATUS_ASYNC_EVENT) {
		...
		if (reason_code == NAN_ASC_EVENT_INIT_CLUSTER) {
			...
			prResponse->u2Length += sizeof(rEvent);

			kalMemCopy(rEvent.aucAnchorMasterRank,
				   prAscAsyncEvt->aucAnchorMasterRank,
				   ANCHOR_MASTER_RANK_NUM);
			COPY_MAC_ADDR(rEvent.aucClusterId,
				      prAscAsyncEvt->ucClusterId);
			COPY_MAC_ADDR(rEvent.aucOwnNMI,
				      prAscAsyncEvt->ucOwnNmi);
			...
			COPY_MAC_ADDR(rEvent.aucOwnNMI,
				      prAscAsyncEvt->ucOwnNmi);
			COPY_MAC_ADDR(rEvent.aucMasterNMI,
				      prAscAsyncEvt->ucMasterNmi);
			...
			struct NAN_ASC_EVENT_DISC_BEACON rEvent = {0};
		...
		nanExtSendIndication(prAdapter, prResponse,
				     EXT_MSG_SIZE(prResponse));
	} else {
		/* ASYNC event doesn't need to clear ASC CMD */
		kalMemCopy(g_nanAscPendingCmd.cmd, prResponse,
			   EXT_MSG_SIZE(prResponse));
	}
