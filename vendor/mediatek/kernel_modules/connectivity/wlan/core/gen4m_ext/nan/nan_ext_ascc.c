				   ...
				   u_int8_t update)
{
	...
	struct IE_NAN_EXT_COMMON_HEADER *h;
	struct IE_NAN_ASCC_CMD *ascc_cmd;
				  ...
				  struct IE_NAN_ASCC_CMD *cmd)
{
	...
	/* Set/clear FR flags accordingly */
	if (cmd->usage == NAN_SCHEDULE_USAGE_ASCC_PS_FR ||
	    cmd->usage == NAN_SCHEDULE_USAGE_APNAN_PS_FR) { /* Power Saving */
		nanExtSetFrPs(prAdapter, prNDL, cmd, TRUE);
	} else if (cmd->usage == NAN_SCHEDULE_USAGE_FR) { /* Fast Recovery */
		nanExtSetFrPs(prAdapter, prNDL, cmd, FALSE);
	} else if (cmd->usage == NAN_SCHEDULE_USAGE_CHECK_SCHEDULE) {
		/* Framework requested to check Fast Recovery status check */
		DBGLOG(NAN, INFO, "Check schedule of req=%u, NMI=" MACSTR,
		       cmd->ucRequestId, MAC2STR(cmd->aucDestNMIAddr));
	}
}
			       ...
			       uint32_t u4SchIdx)
{
	 ...
	 * Set rCustomized[band].ePowerSaving by FR PS or resume
	...
	for (band = NAN_EXT_BAND_2G4, entry_idx = CATEGORY_NDP_FAW_2G;
		...
		if ((ascc_cmd->u4Category & BIT(entry_idx)) == 0)
			continue;
		...
		/* if framework does not set customized FAW for FR  */
		if (ascc_cmd->usage == NAN_SCHEDULE_USAGE_ASCC_PS_FR ||
		    ascc_cmd->usage == NAN_SCHEDULE_USAGE_APNAN_PS_FR) {
			prNdlCustomized[band].ePowerSaving = NDP_ENTER_PS;
		} else if (ascc_cmd->usage == NAN_SCHEDULE_USAGE_FR) {
			prNdlCustomized[band].ePowerSaving = NDP_RESUME_FROM_PS;
		} else { /* Customized after FR, honor fgBitmapSet */
			prNdlCustomized[band].ePowerSaving = NDP_PS_NONE;
		}
		...
		DBGLOG(NAN, INFO,
		       "Set sch=%u band=%u PS=%u %02x:%02x:%02x:%02x:%02x:%02x u4Bitmap=%02x-%02x-%02x-%02x ucOpChannel=%u",
		       u4SchIdx, band,
		       prNdlCustomized[band].ePowerSaving,
		       ascc_cmd->aucDestNMIAddr[0],
		       ascc_cmd->aucDestNMIAddr[1],
		       ascc_cmd->aucDestNMIAddr[2],
		       ascc_cmd->aucDestNMIAddr[3],
		       ascc_cmd->aucDestNMIAddr[4],
		       ascc_cmd->aucDestNMIAddr[5],
		       prSchedEntryNdp[u4SchIdx].byte[0],
		       prSchedEntryNdp[u4SchIdx].byte[1],
		       prSchedEntryNdp[u4SchIdx].byte[2],
		       prSchedEntryNdp[u4SchIdx].byte[3],
		       prSchedEntryNdp[u4SchIdx].ucOpChannel);
	}
}
					 ...
					 struct IE_NAN_ASCC_CMD *cmd)
{
	...
	nanUpdateCustomizedNdpSettings(prNdlCustomized, cmd, NAN_MAX_CONN_CFG);
				       ...
				       struct IE_NAN_ASCC_CMD *cmd)
{
	...
	nanUpdateCustomizedNdpSettings(prNdlCustomized, cmd, u4SchIdx);
					    ...
					    struct IE_NAN_ASCC_CMD *cmd)
{
	...
	if (cmd->usage == NAN_SCHEDULE_USAGE_FR ||
	    cmd->usage == NAN_SCHEDULE_USAGE_ASCC_PS_FR ||
	    cmd->usage == NAN_SCHEDULE_USAGE_APNAN_PS_FR ||
	    cmd->usage == NAN_SCHEDULE_USAGE_CHECK_SCHEDULE) {
		/**
		 * the new Fast Recovery device is successfully resigtered
		 * if a peer matched cmd->aucDestNMIAddr && in FR PS
		 * TODO: NAN_SCHEDULE_USAGE_CHECK_SCHEDULE matching peerNMI
		 */
		nanIndicateActiveFrResponse(prAdapter,
					    cmd->ucRequestId,
					    NAN_FR_PACKET_RESPONSE,
					    NAN_FR_RESPONSE_SUCCESS);
	}

	/**
	 * Run the remembered  reschedule events happened during PS
	 * in nanProcessASCC()
	 */
}
		      ...
		      struct IE_NAN_ASCC_CMD *cmd)
{
	...
	DBGLOG(NAN, INFO, "req=%u method=%u usage=%u category=0x%08x\n",
			...
			size_t u4Size)
{
	...
	if (c->usage == NAN_SCHEDULE_USAGE_ASCC_PS_FR ||
	    c->usage == NAN_SCHEDULE_USAGE_APNAN_PS_FR) {
		if (nanGetActivePeerNum(prAdapter) == 0)
			nanAsccSetFastConnect(prAdapter, FALSE);
	} else if (c->usage == NAN_SCHEDULE_USAGE_FR) {
		if (nanGetActivePeerNum(prAdapter) == 1)
			nanAsccSetFastConnect(prAdapter, TRUE);
		...
		if (prNanScheduler &&
			...
			nanRescheduleNdlIfNeeded(prAdapter,
					prNanScheduler->eRescheduleEventInPS,
					prNanScheduler->prRescheduleNdlInPS);
	}
...
#define NAN_SLOT_IS_M4_CH_SWITCH(_szSlotIdx)\
	...
	if (prAdapter->rWifiVar.ucNanEhtCHSwitchMode == 2 &&
		...
		NAN_DW_DBGLOG(NAN, INFO, fgPrintLog, szSlotIdx,
			      "Tidx(%zu) CH switch slot(%zu): Force empty for M2 CH switch\n",
