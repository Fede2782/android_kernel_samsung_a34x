...
void mt7902_show_wfdma_dbg_probe_info(
	struct ADAPTER *prAdapter,
	enum _ENUM_WFDMA_TYPE_T enum_wfdma_type)
{
	uint16_t u2Idx;
	...
	struct ADAPTER *prAdapter)
{
	...
	mt7902_show_wfdma_dbg_probe_info(prAdapter, WFDMA_TYPE_HOST);
}

#if defined(_HIF_SDIO)
u_int8_t mt7902_sdio_read_wifi_mcu(struct ADAPTER *prAdapter,
	...
	uint8_t ucCase)
{
	...
	if (ucCase == SLAVENORESP) {
		...
		if (!(u4Val & BIT(0)))
			DBGLOG(HAL, INFO, "CONNINFRA off domain bus hng!!!\n");
	...
	uint8_t ucCase)
{
	...
	case SLEEP:
		...
		break;
	case SLAVENORESP:
		DBGLOG(HAL, ERROR, "Slv no response!\n");
#if defined(_HIF_PCIE)
		pcie_mt7902_dump_wfsys_debug_cr(prAdapter, ucCase);
		pcie_mt7902_dump_bgfsys_debug_cr(prAdapter, ucCase);
		pcie_mt7902_dump_conninfra_debug_cr(prAdapter, ucCase);
