	...
	case ETH_P_1X:
	{
		...
		case ETH_EAPOL_KEY: /* key */
			prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter,
				ucBssIndex);
			if (!prBssInfo) {
				DBGLOG(RX, INFO, "BSS Info not exist !!\n");
				return;
			}

			mic_len = wpa_mic_len(prBssInfo->u4RsnSelectedAKMSuite);
			...
			case EVENT_RX:
				if (isPairwise) {
					...
					DBGLOG(RX, INFO,
						"<RX> EAPOL: GTK, M%d, KeyInfo 0x%04x KeyDataLen %d\n",
						m, u2KeyInfo, u2KeyDataLen);
				}
			...
			case EVENT_TX:
				if (isPairwise) {
					...
					DBGLOG(RX, INFO,
						"<TX> EAPOL: GTK, M%d, KeyInfo 0x%04x KeyDataLen %d\n",
						m, u2KeyInfo, u2KeyDataLen);
				}
