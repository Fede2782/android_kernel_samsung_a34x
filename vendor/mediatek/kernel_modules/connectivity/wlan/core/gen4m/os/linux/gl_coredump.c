	...
	struct mt66xx_chip_info *chip_info)
{
	...
	if (glGetRstReason() == RST_FW_ASSERT) {
		coredump_get_dump_buff(aucResetInfo, RST_REPORT_DATA_MAX_LEN);
		sysResetRecordCustomizeData(prGlueInfo, aucResetInfo,
			RST_REPORT_DATA_MAX_LEN, RST_REPORT_NEED_FW_PC_LOG);
	}
#endif
