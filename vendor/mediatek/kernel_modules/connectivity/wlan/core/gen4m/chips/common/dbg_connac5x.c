	...
	uint8_t *tmac_info)
{
	...
	case TMI_HDR_FT_NOR_80211:
		/* HEADER_LENGTH [20:16] */
		DBGLOG(HAL, DEBUG, "\t\t\tHeader Len = %lu(WORD)\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tHeader_padding = %lu\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tTimestamp_offset_index = %lu\n",
	...
	uint32_t u4DmaNum)
{
	...
	DBGLOG(HAL, DEBUG, "\t WFDMA DMA INT STA(0x%08x): 0x%08x\n",
	       u4DmaCfgCrAddr, u4RegValue);
	...
	DBGLOG(HAL, DEBUG, "\t WFDMA DMA INT ENA(0x%08x): 0x%08x\n",
	       u4DmaCfgCrAddr, u4RegValue);
}
	...
	uint32_t u4DmaNum)
{
	...
	DBGLOG(HAL, DEBUG, "WFDMA DMA GLO Config Info:\n");
	...
	enum _ENUM_WFDMA_TYPE_T enum_wfdma_type)
{
	...
	for (idx = 0; idx < group_cnt; idx++) {
		...
		DBGLOG(HAL, DEBUG, "%4d %20s %8x %10llx %8x %6x %6x %6x\n",
			idx,
			group->name,
...
void connac5x_show_mawd_info(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "==============================\n");
	DBGLOG(HAL, DEBUG, " MAWD DEBUG DUMP\n");
	DBGLOG(HAL, DEBUG, "==============================\n");
	...
	DBGLOG(HAL, DEBUG, "ERR_RPT_CTRL2:0x%08x!\n", u4Val);
	while (u4Cidx != u4Didx) {
		if (!pu4ErrRpt)
			break;
		DBGLOG(HAL, DEBUG, "ErrRpt[%d]:0x%08x!\n",
	...
	for (u4Idx = MAWD_HIF_TXD_MD_CTRL0;
		...
		DBGLOG(HAL, DEBUG, "CR [0x%08x]=[0x%08x]",
		       u4Idx + u4MawdOffSet, u4Val);
	}
...
void connac5x_show_rro_info(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "==============================\n");
	DBGLOG(HAL, DEBUG, " RRO DEBUG DUMP\n");
	DBGLOG(HAL, DEBUG, "==============================\n");
	...
	for (u4Idx = 0x100; u4Idx <= 0x10c; u4Idx++) {
		...
		DBGLOG(HAL, DEBUG, "CR [0x%08x]=[0x%08x] [0x%08x]=[0x%08x]",
	...
	if (IS_FEATURE_ENABLED(prWifiVar->fgEnableRroAdvDump)) {
		if (prRxDesc->AllocVa) {
			DBGLOG(HAL, DEBUG, "Dump RxBlkDesc\n");
			DBGLOG_MEM32(HAL, DEBUG, prRxDesc->AllocVa,
				     prRxDesc->AllocSize);
		}

		if (prIndCmd->AllocVa) {
			DBGLOG(HAL, DEBUG, "Dump IndCmd\n");
			DBGLOG_MEM32(HAL, DEBUG, prIndCmd->AllocVa,
				     prIndCmd->AllocSize);
		}

		if (prAddrArray->AllocVa) {
			DBGLOG(HAL, DEBUG, "Dump AddrArray\n");
			DBGLOG_MEM32(HAL, DEBUG, prAddrArray->AllocVa,
			...
			for (u4Idx = 0; u4Idx < u4AddrNum; u4Idx++) {
				...
				if ((u4Addr & 0xf0000000) == 0x90000000) {
					...
					DBGLOG_MEM32(HAL, DEBUG, rAddr, 64);
				}
			}
		}
	}
...
static void connac5x_show_wfdma_wb_info(struct ADAPTER *prAdapter)
{
	...
	if (prRingDmyDbg->AllocVa) {
		...
		DBGLOG_MEM32(HAL, DEBUG, prRingDmyDbg->AllocVa,
			     prRingDmyDbg->AllocSize);
	}
	if (prRingDidx->AllocVa) {
		DBGLOG(HAL, DEBUG, "Dump RingDidx\n");
		DBGLOG_MEM32(HAL, DEBUG, prRingDidx->AllocVa,
			     prRingDidx->AllocSize);
	}
	if (prRingCidx->AllocVa) {
		DBGLOG(HAL, DEBUG, "Dump RingCidx\n");
		DBGLOG_MEM32(HAL, DEBUG, prRingCidx->AllocVa,
			     prRingCidx->AllocSize);
	}
	if (prRingIntSta->AllocVa) {
		u4Val = *((uint32_t *)prRingIntSta->AllocVa);
		DBGLOG(HAL, DEBUG, "EmiIntSta[0x%08x]\n", u4Val);
	}
	if (prHwDoneFlag->AllocVa) {
		DBGLOG(HAL, DEBUG, "Dump HwDoneFlag\n");
		DBGLOG_MEM32(HAL, DEBUG, prHwDoneFlag->AllocVa,
			     prHwDoneFlag->AllocSize);
	}
	if (prSwDoneFlag->AllocVa) {
		DBGLOG(HAL, DEBUG, "Dump SwDoneFlag\n");
		DBGLOG_MEM32(HAL, DEBUG, prSwDoneFlag->AllocVa,
			     prSwDoneFlag->AllocSize);
	}
#if CFG_ENABLE_MAWD_MD_RING
	if (prRingMdDidx->AllocVa) {
		DBGLOG(HAL, DEBUG, "Dump RingMdDidx\n");
		DBGLOG_MEM32(HAL, DEBUG, prRingMdDidx->AllocVa,
...
static void asicConnac5xDmashdlLiteGetMainInfo(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "DMASHDL Main Control (0x%08x): 0x%08x\n",
	       prCfg->rMainControl.u4Addr, u4Val);
...
void connac5x_show_dmashdl_info(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "DMASHDL Counter Check:\n");
	HAL_RMCR_RD(HIF_DBG, prAdapter,
...
void connac5x_show_dmashdl_lite_info(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "DMASHDL lite info:\n");
	...
	DBGLOG(HAL, DEBUG, "PLE/PSE total page (0x%08x): 0x%04x/0x%04x\n",
...
void connac5x_show_ple_info(struct ADAPTER *prAdapter, u_int8_t fgDumpTxd)
{
	...
	DBGLOG(HAL, DEBUG,
	       "\tPacket Buffer Control(0x%08x): 0x%08x\n",
	       WF_PLE_TOP_PBUF_CTRL_ADDR, ple_buf_ctrl);
	...
	for (i = 0; i <= 8; i++) {
		...
		DBGLOG(HAL, DEBUG, "\tPEEK_CR_%02d(0x%08x): 0x%08x\n",
			i, addr, value);
	}
	for (i = 0; i < 2; i++) {
		...
		HAL_RMCR_RD(HIF_DBG, prAdapter, addr, &value);
		DBGLOG(HAL, DEBUG, "\tMACTX0_DBG%d(0x%08x): 0x%08x\n",
			i, addr, value);
	}
	for (i = 0; i < 2; i++) {
		...
		HAL_RMCR_RD(HIF_DBG, prAdapter, addr, &value);
		DBGLOG(HAL, DEBUG, "\tMACTX1_DBG%d(0x%08x): 0x%08x\n",
			i, addr, value);
	}
	DBGLOG(HAL, DEBUG,
	...
	DBGLOG(HAL, DEBUG, "\tFree page counter: 0x%08x\n", pg_flow_ctrl[0]);
	...
	DBGLOG(HAL, DEBUG,
	       "\tFree page head and tail: 0x%08x\n", pg_flow_ctrl[1]);
	...
	DBGLOG(HAL, DEBUG,
	       ...
	       fpg_tail, fpg_head);
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of HIF group: 0x%08x\n",
	       pg_flow_ctrl[2]);
	DBGLOG(HAL, DEBUG,
	       "\tHIF group page status: 0x%08x\n", pg_flow_ctrl[3]);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of WMTXD group: 0x%08x\n",
	       pg_flow_ctrl[8]);
	DBGLOG(HAL, DEBUG,
	       "\tWMTXD group page status: 0x%08x\n", pg_flow_ctrl[9]);
...
void connac5x_show_pse_info(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, DEBUG, "\tPacket Buffer Control: 0x%08x\n", pse_buf_ctrl);
	...
	for (i = 0; i <= 10; i++) {
		...
		HAL_RMCR_RD(HIF_DBG, prAdapter, addr, &value);
		DBGLOG(HAL, DEBUG, "\tSEEK_CR_%02d(0x%08x): 0x%08x\n",
			i, addr, value);
	}
	DBGLOG(HAL, DEBUG,
	...
	DBGLOG(HAL, DEBUG, "\tFree page head and tail: 0x%08x\n", value);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of HIF0 group: 0x%08x\n", hif_grp);
	DBGLOG(HAL, DEBUG, "\tHIF0 group page status: 0x%08x\n", hif_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of HIF1 group: 0x%08x\n", hif_grp);
	DBGLOG(HAL, DEBUG, "\tHIF1 group page status: 0x%08x\n", hif_grp_info);
	...
	DBGLOG(HAL, DEBUG, "\tHIF2 group page status: 0x%08x\n", hif_grp_info);
	...
	DBGLOG(HAL, DEBUG, "\tCPU group page status: 0x%08x\n", cpu_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of LMAC0 group: 0x%08x\n", lmac_grp);
	DBGLOG(HAL, DEBUG,
	       "\tLMAC0 group page status: 0x%08x\n", lmac_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of LMAC1 group: 0x%08x\n", lmac_grp);
	DBGLOG(HAL, DEBUG,
	       "\tLMAC1 group page status: 0x%08x\n", lmac_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of LMAC2 group: 0x%08x\n", lmac_grp);
	DBGLOG(HAL, DEBUG,
	       "\tLMAC2 group page status: 0x%08x\n", lmac_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of LMAC3 group: 0x%08x\n", lmac_grp);
	DBGLOG(HAL, DEBUG,
	       "\tLMAC3 group page status: 0x%08x\n", lmac_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of PLE group: 0x%08x\n", ple_grp);
	DBGLOG(HAL, DEBUG, "\tPLE group page status: 0x%08x\n", ple_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of PLE1 group: 0x%08x\n", ple_grp);
	DBGLOG(HAL, DEBUG, "\tPLE1 group page status: 0x%08x\n", ple_grp_info);
	min_q = (ple_grp & WF_PSE_TOP_PG_PLE_GROUP_PLE_MIN_QUOTA_MASK) >> WF_PSE_TOP_PG_PLE_GROUP_PLE_MIN_QUOTA_SHFT;
	max_q = (ple_grp & WF_PSE_TOP_PG_PLE_GROUP_PLE_MAX_QUOTA_MASK) >> WF_PSE_TOP_PG_PLE_GROUP_PLE_MAX_QUOTA_SHFT;
	DBGLOG(HAL, DEBUG,
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of MDP group: 0x%08x\n", mdp_grp);
	DBGLOG(HAL, DEBUG, "\tMDP group page status: 0x%08x\n", mdp_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of MDP1 group: 0x%08x\n", mdp_grp);
	DBGLOG(HAL, DEBUG, "\tMDP group page status: 0x%08x\n", mdp_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of MDP2 group: 0x%08x\n", mdp_grp);
	DBGLOG(HAL, DEBUG, "\tMDP group page status: 0x%08x\n", mdp_grp_info);
	...
	DBGLOG(HAL, DEBUG,
	       "\tReserved page counter of MDP3 group: 0x%08x\n", mdp_grp);
	DBGLOG(HAL, DEBUG, "\tMDP group page status: 0x%08x\n", mdp_grp_info);
	...
	DBGLOG(HAL, DEBUG, "\tReserved page counter of CPU1 group: 0x%08x\n",
	       cpu_grp);
	DBGLOG(HAL, DEBUG, "\tCPU1 group page status: 0x%08x\n", cpu_grp_info);
	...
	DBGLOG(HAL, DEBUG, "\t\tCPU Q0/1/2/3/4 empty=%d/%d/%d/%d/%d\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tMDP TX Q0/Q1/Q2/RX Q empty=%d/%d/%d/%d\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tSEC TX Q0/Q1/Q2/RX Q empty=%d/%d/%d/%d\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tMDP TXIOC Q0/Q1/Q2 empty=%d/%d/%d\n",
	...
	DBGLOG(HAL, DEBUG, "\t\tMDP RXIOC Q0/Q1/Q2/Q3 empty=%d/%d/%d/%d\n",
	...
	struct SW_RFB *prSwRfb, struct STATS_LLS_WIFI_RATE *rate)
{
	...
	DBGLOG(RX, DEBUG, "Dump RXD:\n");
	DBGLOG_MEM8(RX, DEBUG, prSwRfb->prRxStatus, ad->chip_info->rxd_size);

	DBGLOG(RX, DEBUG, "****** RXD GROUP 3 ******\n");
	DBGLOG_MEM8(RX, DEBUG, prSwRfb->prRxStatusGroup3,
