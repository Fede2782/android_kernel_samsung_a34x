...
#if defined(_HIF_USB)
uint8_t mt7902Connac2xUsbEventEpDetected(struct ADAPTER *prAdapter)
{
...
uint32_t mt7902CoDlImageSectGetPatchInfo(struct ADAPTER *prAdapter,
...
uint32_t mt7902CoDlPatchSendSemaCtl(struct ADAPTER *prAdapter,
				    uint32_t u4Addr,
				    enum ENUM_IMG_DL_IDX_T eDlIdx,
				    uint32_t u4SecInfo,
				    uint8_t *pucSeqNum)
...
uint32_t mt7902CoDlPatchRecvSemaResp(struct ADAPTER *prAdapter,
				    uint8_t ucCmdSeqNum,
				    enum ENUM_IMG_DL_IDX_T eDlIdx,
...
int32_t mt7902CoDlPatchIsDownloaded(struct ADAPTER *prAdapter,
				uint32_t u4DestAddr,
				enum ENUM_IMG_DL_IDX_T eDlIdx,
				uint32_t u4SecInfo,
				uint32_t *u4TargetAddr)
{
...
uint32_t mt7902PatchCoDownload(struct ADAPTER *prAdapter,
					enum ENUM_IMG_DL_IDX_T eDlIdx)
{
	uint32_t u4FwSize = 0;
	uint32_t u4Status;
...
uint32_t mt7902DownloadBtPatch(struct ADAPTER *prAdapter)
{
	uint32_t u4Status;
...
uint32_t mt7902DownloadZbPatch(struct ADAPTER *prAdapter)
{
	uint32_t u4Status;
...
struct wfdma_group_info mt7902_wfmda_host_tx_group[] = {
...
struct wfdma_group_info mt7902_wfmda_host_rx_group[] = {
...
#endif /* _HIF_SDIO */
	.wfmda_host_tx_group = mt7902_wfmda_host_tx_group,
	.wfmda_host_tx_group_len = ARRAY_SIZE(mt7902_wfmda_host_tx_group),
	.wfmda_host_rx_group = mt7902_wfmda_host_rx_group,
	.wfmda_host_rx_group_len = ARRAY_SIZE(mt7902_wfmda_host_rx_group),
};
...
#endif
	.show_wfdma_dbg_probe_info = mt7902_show_wfdma_dbg_probe_info,
};
