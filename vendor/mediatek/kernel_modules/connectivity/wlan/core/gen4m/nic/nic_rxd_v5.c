...
uint16_t nic_rxd_v5_get_rx_byte_count(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}

uint8_t nic_rxd_v5_get_packet_type(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_PKT_TYPE(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_wlan_idx(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_MLD_ID(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_sec_mode(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_SEC_MODE(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_sw_class_error_bit(
	void *prRxStatus)
{
	struct HW_MAC_CONNAC5X_RX_DESC *prRxD;

	prRxD = (struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus;
	if (HAL_MAC_CONNAC5X_RX_STATUS_IS_SW_DEFINE_RX_CLASSERR(prRxD)) {
		DBGLOG(RSN, ERROR,
...
uint8_t nic_rxd_v5_get_ch_num(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_CHNL_NUM(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_rf_band(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_RF_BAND(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_tcl(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_TCL(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
...
uint8_t nic_rxd_v5_get_ofld(
	void *prRxStatus)
{
	return HAL_MAC_CONNAC5X_RX_STATUS_GET_OFLD(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
				   ...
				   struct SW_RFB *prSwRfb)
{
	...
	struct HW_MAC_CONNAC5X_RX_DESC *prRxStatus = prSwRfb->prRxStatus;
	...
	if (RXM_IS_MGMT_FRAME(u2FrameCtrl))
		...
		(uint8_t)HAL_MAC_CONNAC5X_RX_STATUS_GET_BSSID(prRxStatus),
		(uint8_t)HAL_MAC_CONNAC5X_RX_STATUS_GET_MLD_ID(prRxStatus));

	else
		return getPrimaryWlanIdx(prAdapter, prSwRfb->ucTid, (uint8_t)
			HAL_MAC_CONNAC5X_RX_STATUS_GET_MLD_ID(prRxStatus));
}
...
static uint8_t getWlanIndex(struct ADAPTER *prAdapter, struct SW_RFB *prSwRfb)
{
	...
	if (prSwRfb->fgIsBC || prSwRfb->fgIsMC)
		return HAL_MAC_CONNAC5X_RX_STATUS_GET_MLD_ID(prRxStatus);
	else
 ...
 */
/*----------------------------------------------------------------------------*/
void nic_rxd_v5_fill_rfb(struct ADAPTER *prAdapter, struct SW_RFB *prSwRfb)
{
	...
	u4PktLen = (uint32_t) HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(
				prRxStatus);
	u4HeaderOffset = (uint32_t) (
		HAL_MAC_CONNAC5X_RX_STATUS_GET_HEADER_OFFSET(prRxStatus));

	u2RxStatusOffset = prChipInfo->rxd_size;
	prSwRfb->ucGroupVLD =
		(uint8_t) HAL_MAC_CONNAC5X_RX_STATUS_GET_GROUP_VLD(prRxStatus);
...
#define RETRY_BIT 3
	...
	prSwRfb->u2HeaderLen = (uint16_t)
		HAL_MAC_CONNAC5X_RX_STATUS_GET_HEADER_LEN(prRxStatus);

	prSwRfb->ucHwBandIdx =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_BAND_IDX(prRxStatus);
	prSwRfb->ucHwBssIdx =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_BSSID(prRxStatus);

	prSwRfb->fgIsBC = HAL_MAC_CONNAC5X_RX_STATUS_IS_BC(prRxStatus);
	prSwRfb->fgIsMC = HAL_MAC_CONNAC5X_RX_STATUS_IS_MC(prRxStatus);
	/* For MLO, MLD_ID need to be corrected according to TID */
	prSwRfb->ucTid =
		(uint8_t) HAL_MAC_CONNAC5X_RX_STATUS_GET_TID(prRxStatus);
	...
	prSwRfb->prStaRec = cnmGetStaRecByIndex(prAdapter,
			prSwRfb->ucStaRecIdx);
	prSwRfb->fgHdrTran =
		HAL_MAC_CONNAC5X_RX_STATUS_IS_HEADER_TRAN(prRxStatus);
	prSwRfb->ucPayloadFormat =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_PAYLOAD_FORMAT(prRxStatus);
	prSwRfb->ucRxClassify =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_CLASSIFY(prRxStatus);

	prSwRfb->fgIcvErr =
		HAL_MAC_CONNAC5X_RX_STATUS_IS_ICV_ERROR(prRxStatus);
	prSwRfb->ucSecMode =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_SEC_MODE(prRxStatus);
	prSwRfb->ucOFLD = HAL_MAC_CONNAC5X_RX_STATUS_GET_OFLD(prRxStatus);
	prSwRfb->fgIsCipherMS =
		HAL_MAC_CONNAC5X_RX_STATUS_IS_CIPHER_MISMATCH(prRxStatus);
	prSwRfb->fgIsCipherLenMS =
		HAL_MAC_CONNAC5X_RX_STATUS_IS_CLM_ERROR(prRxStatus);
	prSwRfb->fgIsFrag = HAL_MAC_CONNAC5X_RX_STATUS_IS_FRAG(prRxStatus);
	prSwRfb->fgIsFCS = HAL_MAC_CONNAC5X_RX_STATUS_IS_FCS_ERROR(prRxStatus);
	prSwRfb->fgIsAmpdu = HAL_MAC_CONNAC5X_RX_STATUS_IS_NAMP(prRxStatus);
	prSwRfb->ucRxvSeqNo =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_RXV_SEQ_NO(prRxStatus);
	ucHwChnlNum = HAL_MAC_CONNAC5X_RX_STATUS_GET_CHNL_NUM(prRxStatus);
	prSwRfb->eRfBand =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_RF_BAND(prRxStatus);
	prSwRfb->ucChnlNum =
		nicRxdChNumTranslate(prSwRfb->eRfBand, ucHwChnlNum);
	prSwRfb->ucTcl =
		HAL_MAC_CONNAC5X_RX_STATUS_GET_TCL(prRxStatus);
	...
	if (IS_FEATURE_ENABLED(prWifiVar->fgEnableRro)) {
		prSwRfb->u4TcpUdpIpCksStatus =
			HAL_MAC_CONNAC5X_RX_STATUS_GET_CHECKSUM(prRxStatus);
	...
	struct SW_RFB *prSwRfb)
{
	...
	prChipInfo = prAdapter->chip_info;
	prRxStatus = (struct HW_MAC_CONNAC5X_RX_DESC *)prSwRfb->prRxStatus;
	...
	if (!HAL_MAC_CONNAC5X_RX_STATUS_IS_FCS_ERROR(prRxStatus)
	    && !HAL_MAC_CONNAC5X_RX_STATUS_IS_DAF(prRxStatus)
	    && !HAL_MAC_CONNAC5X_RX_STATUS_IS_ICV_ERROR(prRxStatus)
		...
		if (!HAL_MAC_CONNAC5X_RX_STATUS_IS_ICV_ERROR(prRxStatus)
		    && HAL_MAC_CONNAC5X_RX_STATUS_IS_TKIP_MIC_ERROR(
			prRxStatus)) {
		...
		} else if (HAL_MAC_CONNAC5X_RX_STATUS_IS_LLC_MIS(prRxStatus)
			 && !HAL_MAC_CONNAC5X_RX_STATUS_IS_ERROR(prRxStatus)
			 && !HAL_MAC_CONNAC5X_RX_STATUS_IS_FCS_ERROR(prRxStatus)
	...
	if (fgDrop) {
		...
		if (HAL_MAC_CONNAC5X_RX_STATUS_IS_DAF(prRxStatus))
...
#if CFG_SUPPORT_FRAG_AGG_VALIDATION
		if (HAL_MAC_CONNAC5X_RX_STATUS_IS_TKIP_MIC_ERROR(prRxStatus))
...
{
	return HAL_MAC_CONNAC5X_RX_STATUS_IS_HEADER_TRAN(
		(struct HW_MAC_CONNAC5X_RX_DESC *)prRxStatus);
}
	...
	struct SW_RFB *prSwRfb)
{
	...
	prSwRfb->fgHdrTran = nic_rxd_v5_get_HdrTrans(prRxStatus);
	prSwRfb->ucOFLD = nic_rxd_v5_get_ofld(prRxStatus);
	...
	/* HIF_RX_PKT_TYPE_EVENT */
		if ((NIC_RX_GET_U2_SW_PKT_TYPE(prSwRfb->prRxStatus) &
			CONNAC5X_RX_STATUS_PKT_TYPE_SW_BITMAP) ==
			CONNAC5X_RX_STATUS_PKT_TYPE_SW_EVENT) {
			...
			CONNAC5X_RX_STATUS_PKT_TYPE_SW_BITMAP) ==
			CONNAC5X_RX_STATUS_PKT_TYPE_SW_FRAME) {
...
/* fos_change begin */
			...
			u2PktLen =
				HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(
				prRxStatus);

			u4HeaderOffset = (uint32_t)
				HAL_MAC_CONNAC5X_RX_STATUS_GET_HEADER_OFFSET(
				prRxStatus);
			if (prSwRfb->ucGroupVLD & BIT(RX_GROUP_VLD_4))
...
/* fos_change begin */
		...
		u2PktLen =
			HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(prRxStatus);
		u4HeaderOffset = (uint32_t)
			HAL_MAC_CONNAC5X_RX_STATUS_GET_HEADER_OFFSET(
				prRxStatus);
		...
		if (HAL_MAC_CONNAC5X_RX_STATUS_IS_NDATA(prRxStatus)) {
		...
		default:
			...
			if (HAL_MAC_CONNAC5X_RX_STATUS_IS_HEADER_TRAN(
						prRxStatus)) {
				if (HAL_MAC_CONNAC5X_RX_STATUS_IS_LLC_MIS(
						prRxStatus)) {
					DBGLOG(RX, DEBUG,
					...
					ucPfSts =
					HAL_MAC_CONNAC5X_RX_STATUS_GET_PF_STS(
						prRxStatus);
					DBGLOG(RX, DEBUG,
	...
	struct SW_RFB *prSwRfb)
{
	...
	prSwRfb->u2RxByteCount =
		(uint16_t) HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(
			prRxStatus);
	...
	if (HAL_MAC_CONNAC5X_RX_STATUS_GET_RXV_SEQ_NO(prRxStatus) != 0)
	...
	prRadiotapInfo->ucFcsErr =
		HAL_MAC_CONNAC5X_RX_STATUS_IS_FCS_ERROR(prRxStatus);
	prRadiotapInfo->ucFrag = HAL_MAC_CONNAC5X_RX_STATUS_IS_FRAG(prRxStatus);
	ucHwChnlNum = HAL_MAC_CONNAC5X_RX_STATUS_GET_CHNL_NUM(prRxStatus);
	...
	prRadiotapInfo->ucTxMode =
		HAL_MAC_CONNAC5X_RX_VT_GET_RX_MODE(prRxStatusGroup5);
	prRadiotapInfo->ucFrMode =
		HAL_MAC_CONNAC5X_RX_VT_GET_FR_MODE(prRxStatusGroup5);
	prRadiotapInfo->ucShortGI =
		HAL_MAC_CONNAC5X_RX_VT_GET_SHORT_GI(prRxStatusGroup5);
	prRadiotapInfo->ucSTBC =
		HAL_MAC_CONNAC5X_RX_VT_GET_STBC(prRxStatusGroup5);
	prRadiotapInfo->ucNess =
		HAL_MAC_CONNAC5X_RX_VT_GET_NESS(prRxStatusGroup5);
	prRadiotapInfo->ucLDPC =
		HAL_MAC_CONNAC5X_RX_VT_GET_LDPC(prRxStatusGroup3);
	prRadiotapInfo->ucMcs =
		HAL_MAC_CONNAC5X_RX_VT_GET_RX_RATE(prRxStatusGroup3);
	prRadiotapInfo->ucRcpi0 =
		HAL_MAC_CONNAC5X_RX_VT_GET_RCPI0(prRxStatusGroup5);
	prRadiotapInfo->ucRcpi1 =
		HAL_MAC_CONNAC5X_RX_VT_GET_RCPI1(prRxStatusGroup5);
	prRadiotapInfo->ucTxopPsNotAllow =
		HAL_MAC_CONNAC5X_RX_VT_TXOP_PS_NOT_ALLOWED(prRxStatusGroup5);
	prRadiotapInfo->ucLdpcExtraOfdmSym =
		HAL_MAC_CONNAC5X_RX_VT_LDPC_EXTRA_OFDM_SYM(prRxStatusGroup5);
	prRadiotapInfo->ucVhtGroupId =
		HAL_MAC_CONNAC5X_RX_VT_GET_GROUP_ID(prRxStatusGroup5);
	prRadiotapInfo->ucNsts =
		HAL_MAC_CONNAC5X_RX_VT_GET_NSTS(prRxStatusGroup3) + 1;
	prRadiotapInfo->ucBeamFormed =
		HAL_MAC_CONNAC5X_RX_VT_GET_BEAMFORMED(prRxStatusGroup3);

	if (prRadiotapInfo->ucTxMode >= TX_RATE_MODE_HE_SU) {
		prRadiotapInfo->ucPeDisamb =
			HAL_MAC_CONNAC5X_RX_VT_GET_PE_DIS_AMB(prRxStatusGroup5);
		prRadiotapInfo->ucNumUser =
			HAL_MAC_CONNAC5X_RX_VT_GET_NUM_USER(prRxStatusGroup5);
		prRadiotapInfo->ucSigBRU0 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SIGB_RU0(prRxStatusGroup5);
		prRadiotapInfo->ucSigBRU1 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SIGB_RU1(prRxStatusGroup5);
		prRadiotapInfo->ucSigBRU2 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SIGB_RU2(prRxStatusGroup5);
		prRadiotapInfo->ucSigBRU3 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SIGB_RU3(prRxStatusGroup5) |
			HAL_MAC_CONNAC5X_RX_VT_GET_SIGB_RU3_1(prRxStatusGroup5);
		prRadiotapInfo->u2VhtPartialAid =
			HAL_MAC_CONNAC5X_RX_VT_GET_PART_AID(prRxStatusGroup5);
		prRadiotapInfo->u2RuAllocation =
			HAL_MAC_CONNAC5X_RX_VT_GET_RU_ALLOC(prRxStatusGroup3);
		prRadiotapInfo->u2BssClr =
			HAL_MAC_CONNAC5X_RX_VT_GET_BSS_COLOR(prRxStatusGroup5);
		prRadiotapInfo->u2BeamChange =
			HAL_MAC_CONNAC5X_RX_VT_GET_BEAM_CHANGE(
				prRxStatusGroup5);
		prRadiotapInfo->u2UlDl =
			HAL_MAC_CONNAC5X_RX_VT_GET_UL_DL(prRxStatusGroup5);
		prRadiotapInfo->u2DataDcm =
			HAL_MAC_CONNAC5X_RX_VT_GET_DCM(prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse1 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SPATIAL_REUSE1(
				prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse2 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SPATIAL_REUSE2(
				prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse3 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SPATIAL_REUSE3(
				prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse4 =
			HAL_MAC_CONNAC5X_RX_VT_GET_SPATIAL_REUSE4(
				prRxStatusGroup5);
		prRadiotapInfo->u2Ltf =
			HAL_MAC_CONNAC5X_RX_VT_GET_LTF(prRxStatusGroup5) + 1;
		prRadiotapInfo->u2Doppler =
			HAL_MAC_CONNAC5X_RX_VT_GET_DOPPLER(prRxStatusGroup5);
		prRadiotapInfo->u2Txop =
			HAL_MAC_CONNAC5X_RX_VT_GET_TXOP(prRxStatusGroup5);
		prRadiotapInfo->ucPpduTypeComp =
			HAL_MAC_CONNAC5X_RX_VT_GET_PPDU_TYPE_COMP(
				prRxStatusGroup5);
		prRadiotapInfo->ucEhtSigMcs =
			HAL_MAC_CONNAC5X_RX_VT_GET_EHT_SIG_MCS(
				prRxStatusGroup5);
		prRadiotapInfo->ucEhtLtfSymNum =
			HAL_MAC_CONNAC5X_RX_VT_GET_EHT_LTF_SYM_NUM(
				prRxStatusGroup5);
		prRadiotapInfo->ucEhtSigSymNum =
			HAL_MAC_CONNAC5X_RX_VT_GET_EHT_SIG_SYM_NUM(
				prRxStatusGroup5);
	}
