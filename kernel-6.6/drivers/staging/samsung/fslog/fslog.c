...
#define FSLOG_FILE_MODE_VERSION	(0444)
#define FSLOG_FILE_MODE_DIR	(0555)
#define FSLOG_FILE_MODE_STLOG	(0664)
#define FSLOG_FILE_MODE_DLOG	(0440)
...
#define DEFINE_FSLOG_VARIABLE(_name, _size) \
static char fslog_cbuf_##_name[_size]; \
\
...
static const char FSLOG_VERSION_STR[] = "1.1.1\n";
...
static int __init fslog_init(void)
{
	...
	proc_create_single("version", 0, fslog_dir, fslog_version_show);
...
fs_initcall(fslog_init);
module_exit(fslog_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Storage log buffers");
