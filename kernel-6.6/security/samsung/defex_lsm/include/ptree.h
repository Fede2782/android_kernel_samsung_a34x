...
/* Flags for PPTree.behavior mask, for compatibility when a tree generated by
 * out-of-date tooling is loaded (not a desirable situation, but possible since
 * the producer and consumer of PPTree reside in different source trees and
 * builds could be misconfigured).
 */
#define PPB_SPLIT_WC_KEYS 1 // string keys with wildcards are at the end

/* Magic number (fixed portion, plus two version bytes) */
#define PPTREE_MAGIC "PPTree-\1\1"
#define PPTREE_MAGIC_FIXEDSIZE 7

/* Queries version of binary data as second-least significant (major version)
 * and least significant (minor version) bytes of result
 */
extern unsigned int pptree_get_version(const struct PPTree *tree);
/* Returns expected version of binary data as second-least significant (major
 * version) and least significant (minor version) bytes of result
 */
extern unsigned int pptree_expected_version(void);
